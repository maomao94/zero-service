...
// Embedder 将文本编码为向量（与 eino-ext Embedding 组件职责一致，此处为 HTTP 直连 ARK OpenAI 兼容接口）。
...
// Embed 批量调用；单次请求 input 条数不宜过大。
func (e *ArkEmbedder) Embed(ctx context.Context, texts []string) ([][]float32, error) {
	if e.apiKey == "" {
		return nil, fmt.Errorf("rag embedding: api key is empty")
	}
	if e.model == "" {
		return nil, fmt.Errorf("rag embedding: model (endpoint id) is empty")
	}
	const batch = 16
	var out [][]float32
...
func (e *ArkEmbedder) embedBatch(ctx context.Context, texts []string) ([][]float32, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("rag embedding: http %d: %s", resp.StatusCode, string(raw))
	}
	var er embeddingsResponse
	if err := json.Unmarshal(raw, &er); err != nil {
		return nil, fmt.Errorf("rag embedding: decode: %w", err)
	}
	if er.Error != nil && er.Error.Message != "" {
		return nil, fmt.Errorf("rag embedding: %s", er.Error.Message)
	}
	if len(er.Data) != len(texts) {
		return nil, fmt.Errorf("rag embedding: expect %d vectors, got %d", len(texts), len(er.Data))
	}
