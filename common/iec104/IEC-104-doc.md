...
- **自定义Topic生成**：基于配置规则和消息元数据动态生成推送Topic
...
本次重构主要实现了以下架构升级：
1. 引入三协议支持，同时支持Kafka、MQTT和gRPC消息推送
2. 实现自定义Topic生成机制，支持基于配置和元数据的灵活路由
3. 完善配置系统，支持全配置化部署

### 0.7 典型应用场景

- **智能电网监控**：实时采集电网设备数据，通过双协议推送至数据平台
- **工业自动化**：实现工业现场设备的远程监控和控制
- **新能源监控**：采集光伏、风电等新能源设备数据，支持多协议数据上报
- **智能楼宇**：实现楼宇设备的集中监控和管理
- **智慧城市**：支持城市基础设施的实时监控和数据分析

#### 架构特点

1. **分层设计**：核心处理层与输出层分离，便于扩展新的输出协议
2. **多协议支持**：同时支持 Kafka、MQTT 和 gRPC 输出，满足不同场景需求
3. **高性能**：采用连接池等技术优化性能
4. **可靠性**：支持自动重连、消息确认机制
5. **易扩展**：模块化设计，便于添加新的协议支持
...
- **传输载体**：Kafka/MQTT Topic/gRPC（具体名称协商约定）
- **数据格式**：JSON
- **编码格式**：UTF-8
