...
本项目的 ISP 模块面向变电站远程智能巡视场景，提供两个服务：

| 服务 | 端口 | TCP 角色 | 说明 |
|------|------|----------|------|
| `ispagent.rpc` | gRPC `21014` | ISP TCP Client | 主动连接上级巡视系统，接收任务/模型/控制指令，并向上级上报巡视数据 |
| `ispserver.rpc` | gRPC `21015` + TCP `7100` | ISP TCP Server | 监听下级设备或站端系统连接，处理注册、心跳和已登记的上行消息 |
...
| 对接层 | `ispagent` / `ispserver` | 复用 `common/isp` 的 TCP Client/Server、注册、心跳、消息编解码和请求应答能力 |

两个服务共享 `common/isp` 协议基础包，业务包只注册 Type/Command handler，不直接处理 gnetx 编解码和会话细节。
...
|------|---------|------|------|------|
| 251 | 1 | 注册 | 双向角色相关 | `ispagent` 作为 client 建连后主动注册；`ispserver` 作为 server 接收下级注册 |
| 251 | 2 | 心跳 | 双向角色相关 | `ispagent` 周期发送心跳；`ispserver` 接收心跳并应答 |
...
`ispserver` 暴露当前 ISP TCP 服务端会话管理接口：

| RPC | 说明 |
|-----|------|
| `ListSessions` | 查询当前已连接/已注册的下级设备会话 |
| `DisconnectSession` | 按 session ID 断开指定下级设备连接 |
