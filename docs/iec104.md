...
平台默认使用 gRPC 协议进行 IEC 104 消息转发，协议定义在 `streamevent.proto` 文件中。该协议支持将 IEC 104 消息转换为统一格式，并通过 gRPC 接口进行推送，同时支持 mqtt/kafka 协议进行数据推送。
...
**分层拦截策略**：
| 场景 | enable_push | enable_raw_insert | 行为 |
|------|-------------|-------------------|------|
| 1 | 0 | 0 | caller 不推送，数据不会到达 facade |
| 2 | 0 | 1 | caller 不推送，数据不会到达 facade |
| 3 | 1 | 0 | caller 推送，facade 层不插入 TDengine |
| 4 | 1 | 1 | caller 推送，facade 层插入 TDengine |
