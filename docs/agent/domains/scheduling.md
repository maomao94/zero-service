...
修改 `common/crontask` 的 Scheduler、Task/Handler、Store、lease、`RunNow`，或实现 Trigger/ISP 存储适配器时读取。RRULE 解析、平移、批量查询和中文描述见 [rrulex RRULE 扩展规范](./rrule.md)。
...
- RDATE/EXDATE、DTSTART/INTERVAL、DST 与安全平移的算法契约及测试要求见 `rrule.md`，不能在 Scheduler 层复制第二套实现。
