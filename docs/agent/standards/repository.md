...
新增目录、移动代码、选择复用位置、判断文件是否可直接编辑或决定是否需要独立领域文档时读取。
...
- 小型纯函数工具包由相邻代码、测试和 [公共包设计](./shared-packages.md) 覆盖，不机械创建“一包一文档”。

实验代码变成稳定基础设施后，必须先证明存在正式调用方、清晰所有者、生命周期和测试，再决定是否补充领域文档。
