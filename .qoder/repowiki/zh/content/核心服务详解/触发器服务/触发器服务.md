...
- 逻辑层：sendtriggerlogic 实现异步触发；createplantasklogic 实现计划任务创建。
...
LOGIC2["创建计划任务逻辑<br/>createplantasklogic.go"]
...
## 架构总览
触发器服务采用“gRPC API + Asynq 异步队列 + 数据库状态机”的架构模式：
...
**图表来源**
- [asynqTaskServer.go](file://common/asynqx/asynqTaskServer.go)
- [asynqSchedulerServer.go](file://common/asynqSchedulerServer.go)
- [asynqClient.go](file://common/asynqx/asynqClient.go)
- [tasktype.go](file://common/asynqx/tasktype.go)
...
- Asynq 客户端/服务器/调度器：参考 [asynqClient.go](file://common/asynqx/asynqClient.go)、[asynqTaskServer.go](file://common/asynqx/asynqTaskServer.go)、[asynqSchedulerServer.go](file://common/asynq/asynqSchedulerServer.go)
