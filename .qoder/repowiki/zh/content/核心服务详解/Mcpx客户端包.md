...
## 更新摘要
**变更内容**
- 新增用户上下文提取功能（WithExtractUserCtx选项），允许从_Meta上下文中自动提取用户身份信息（user-id, user-name等），并将其注入到Go上下文值中
- 该功能保持向后兼容性，同时为需要用户身份访问的应用提供可选的增强功能
- 增强了每消息认证机制，支持更精确的用户上下文传递
- 改进了工具调用包装器，提供更灵活的用户上下文处理选项
...
**更新** 该版本引入了全新的用户上下文提取功能（WithExtractUserCtx选项），允许从_JSON-RPC请求的_Meta字段中自动提取用户身份信息（user-id, user-name等），并将其注入到Go上下文值中，使业务层能够访问用户身份信息。此增强功能保持向后兼容性，同时为需要用户身份访问的应用提供可选的增强功能。

**新增** WithCtxProp函数中增强了工具调用日志记录功能，现在会在每次工具调用时记录会话ID和请求参数信息，显著提升了系统的可观测性和调试能力。日志记录包括会话标识符和请求参数详情，便于追踪工具调用的完整生命周期。
...
subgraph "工具实现"
AP[工具注册] --> AQ[CallToolWrapper<br/>工具包装器]
AR[进度发送器] --> AS[ProgressSenderContext<br/>进度上下文]
end
```
...
- [wrapper.go:1-191](file://common/mcpx/wrapper.go#L1-L191)
...
subgraph "Mcpx客户端层"
D[Client<br/>客户端管理器]
E[serverConn<br/>服务器连接]
F[工具聚合<br/>路由管理]
G[每消息认证<br/>上下文注入]
H[用户上下文提取<br/>WithExtractUserCtx]
I[工具调用日志<br/>可观测性增强]
J[CallToolWrapper<br/>工具包装器]
end
subgraph "统一传输层"
K[Streamable HTTP<br/>统一传输协议]
L[SSE传输<br/>兼容模式]
M[HTTP客户端<br/>ctxHeaderTransport]
N[增强SSE传输<br/>authSSEHandler]
O[SetMeta注入<br/>JSON-RPC请求]
P[ExtractFromMeta注入<br/>用户上下文提取]
end
subgraph "双模式认证层"
Q[ServiceToken验证器<br/>连接级认证]
R[JWT解析器<br/>用户级认证]
S[Claims提取<br/>用户信息解析]
T[SSE认证增强<br/>会话管理]
U[用户上下文提取<br/>ExtractFromMeta]
end
subgraph "增强上下文传播层"
V[HTTP头传播<br/>MCP客户端]
W[gRPC元数据传播<br/>RPC客户端拦截器]
X[流式RPC处理<br/>服务端拦截器]
Y[SSE传输Fallback<br/>认证类型提取]
Z[增强日志输出<br/>认证类型记录]
AA[每消息上下文<br/>SetMeta注入]
BB[工具调用追踪<br/>会话ID记录]
CC[用户身份透传<br/>gRPC元数据注入]
end
subgraph "MCP服务器层"
DD[McpServer<br/>服务器封装]
EE[工具注册<br/>Echo/Modbus]
FF[认证中间件<br/>RequireBearerToken]
GG[SSE认证处理器<br/>authSSEHandler]
HH[CallToolWrapper<br/>工具包装器]
II[WithExtractUserCtx<br/>用户上下文提取]
end
A --> D
D --> E
D --> F
...
subgraph "内部模块"
K[common/mcpx/client.go<br/>客户端实现]
L[common/mcpx/auth.go<br/>双模式认证]
M[common/mcpx/server.go<br/>服务器封装]
N[common/mcpx/wrapper.go<br/>工具包装器]
O[common/mcpx/ctxprop.go<br/>上下文属性]
P[common/mcpx/sse_auth.go<br/>SSE认证增强]
Q[common/ctxdata/ctxData.go<br/>上下文字段定义]
R[common/ctxprop/claims.go<br/>Claims处理]
S[common/ctxprop/http.go<br/>HTTP传播]
T[common/ctxprop/ctx.go<br/>上下文收集]
U[common/Interceptor/rpcclient/metadataInterceptor.go<br/>RPC客户端拦截器]
V[common/Interceptor/rpcserver/loggerInterceptor.go<br/>RPC服务端拦截器]
W[common/tool/tool.go<br/>工具函数]
X[common/mcpx/logger.go<br/>日志适配]
end
subgraph "AI应用"
Y[aiapp/aichat<br/>聊天应用]
Z[aiapp/mcpserver<br/>MCP服务器]
AA[aiapp/mcpserver/internal/tools<br/>工具实现]
BB[echo.go<br/>回显工具使用WithExtractUserCtx]
CC[modbus.go<br/>Modbus工具使用WithExtractUserCtx]
end
subgraph "用户上下文提取"
DD[wrapper.go:28-35<br/>WithExtractUserCtx选项]
EE[wrapper.go:133-137<br/>ExtractFromMeta注入]
FF[ctxprop.go:28-41<br/>ExtractFromMeta函数]
GG[ctxprop.go:10-11<br/>CollectFromCtx函数]
HH[ctxData.go:5-12<br/>CtxMetaKey定义]
end
subgraph "链路追踪"
II[wrapper.go:119-125<br/>ExtractTraceFromMeta]
JJ[ctxprop.go:43-51<br/>ExtractTraceFromMeta函数]
KK[I:8-9<br/>otel.TextMapPropagator导入]
LL[J:1-1<br/>propagation.TextMapCarrier接口]
end
```
...
- **新增** 用户上下文提取功能优化，减少重复解析_meta字段的开销
...
通过其可靠的设计、完善的实现和现代化的功能增强，Mcpx客户端包为Zero Service项目提供了一个强大而灵活的MCP客户端解决方案，特别适合需要高级安全控制、复杂上下文管理和多协议通信的企业级应用场景。新增的每消息认证机制、SSE认证增强系统、用户上下文提取功能和增强的工具调用日志记录功能进一步提升了系统的兼容性、可靠性和可观测性，为实时数据流应用和精确认证控制提供了更好的支持，同时也为生产环境的运维和审计提供了强有力的技术保障。