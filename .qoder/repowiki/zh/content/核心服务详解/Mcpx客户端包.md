...
- **新增** WithCtxProp函数中增强了工具调用日志记录功能，记录会话ID和请求参数信息，提升系统可观测性
...
subgraph "统一传输层"
H[Streamable HTTP<br/>统一传输协议]
I[SSE传输<br/>兼容模式]
J[HTTP客户端<br/>ctxHeaderTransport]
K[增强SSE传输<br/>authSSEHandler]
L[SetMeta注入<br/>JSON-RPC请求]
end
subgraph "双模式认证层"
M[ServiceToken验证器<br/>连接级认证]
N[JWT解析器<br/>用户级认证]
O[Claims提取<br/>用户信息解析]
P[SSE认证增强<br/>会话管理]
end
subgraph "增强上下文传播层"
Q[HTTP头传播<br/>MCP客户端]
R[gRPC元数据传播<br/>RPC客户端拦截器]
S[流式RPC处理<br/>服务端拦截器]
T[SSE传输Fallback<br/>认证类型提取]
U[增强日志输出<br/>认证类型记录]
V[每消息上下文<br/>SetMeta注入]
W[工具调用追踪<br/>会话ID记录]
end
subgraph "MCP服务器层"
X[McpServer<br/>服务器封装]
Y[工具注册<br/>Echo/Modbus]
Z[认证中间件<br/>RequireBearerToken]
AA[SSE认证处理器<br/>authSSEHandler]
end
A --> C
C --> D
C --> E
...
**新增** 架构中增加了工具调用日志记录层，通过WithCtxProp函数的增强日志功能，提供完整的工具调用追踪能力，包括会话ID和请求参数的详细记录。
...
**新增** WithCtxProp函数的增强日志记录功能为系统提供了强大的可观测性支持，通过记录会话ID和请求参数信息，开发者可以轻松追踪工具调用的完整生命周期，这对于调试、监控和审计都是极其重要的。

通过其可靠的设计、完善的实现和现代化的功能增强，Mcpx客户端包为Zero Service项目提供了一个强大而灵活的MCP客户端解决方案，特别适合需要高级安全控制、复杂上下文管理和多协议通信的企业级应用场景。新增的每消息认证机制、SSE认证增强系统和增强的工具调用日志记录功能进一步提升了系统的兼容性、可靠性和可观测性，为实时数据流应用和精确认证控制提供了更好的支持，同时也为生产环境的运维和审计提供了强有力的技术保障。