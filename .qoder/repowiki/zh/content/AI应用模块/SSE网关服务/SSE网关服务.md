...
## 更新摘要
**变更内容**
- 基于应用变更：SSE网关服务与AI聊天服务的工具调用事件系统协同工作，提供完整的流式事件传输支持
- 新增工具事件的SSE事件类型处理和实时进度展示功能
- 扩展AI聊天流处理逻辑，支持工具调用事件的实时传输和客户端渲染
- 增强聊天界面的工具事件处理能力，提供可视化进度展示
...
## 简介
本项目为一个基于 Go-Zero 的 Server-Sent Events（SSE）网关服务，提供三类实时流能力：
- SSE 事件流：用于推送系统通知与状态变更等事件。
- AI 对话流：以流式方式逐字返回生成的 token，模拟大模型推理过程。
- **更新** 工具调用事件流：实时传输工具调用的开始、进度、成功和错误状态，提供完整的工具执行可视化。
...
- AI聊天服务（aiapp/aichat）：提供工具调用事件的实时传输和处理能力
...
## 项目结构
SSE 网关服务位于 aiapp/ssegtw 目录，AI 网关服务位于 aiapp/aigtw 目录，AI聊天服务位于 aiapp/aichat 目录，主要由以下层次组成：
- 入口与配置：应用入口、配置加载、跨域与服务启动
- 路由与处理器：REST 路由注册、SSE 路由启用、HTTP 处理器
- 业务逻辑：SSE 事件流、AI 对话流和工具调用事件流的具体实现
- 类型定义：请求与响应类型
- 服务上下文：RPC 客户端、事件发射器、等待完成注册
- SSE 写入器：封装 SSE 协议写入与自动刷新
- 静态文件服务：提供聊天界面 HTML 文件
- **更新** 工具调用事件系统：支持工具调用的实时进度传输和客户端渲染
...
L --> N["模型列表处理器<br/>internal/handler/pass/listModelsHandler.go"]
L --> O["对话补全处理器<br/>internal/handler/pass/chatCompletionsHandler.go"]
N --> P["模型列表逻辑<br/>internal/logic/pass/listModelsLogic.go"]
O --> Q["对话补全逻辑<br/>internal/logic/pass/chatCompletionsLogic.go"]
Q --> R["AI服务上下文<br/>internal/svc/servicecontext.go"]
S["AI聊天服务"] --> T["聊天完成流逻辑<br/>aiapp/aichat/internal/logic/chatcompletionstreamlogic.go"]
T --> U["工具事件定义<br/>ToolEvent结构体"]
T --> V["工具事件发送<br/>sendToolEvent函数"]
U --> W["SSE工具事件类型<br/>tool_start/tool_progress/tool_success/tool_error"]
X["聊天界面"] --> Y["工具事件处理<br/>handleToolEvent函数"]
Y --> Z["工具卡片渲染<br/>renderToolCards函数"]
```
...
- [aiapp/aichat/internal/logic/chatcompletionstreamlogic.go:23-54](file://aiapp/aichat/internal/logic/chatcompletionstreamlogic.go#L23-L54)
- [aiapp/aigtw/chat.html:2359-2513](file://aiapp/aigtw/chat.html#L2359-2513)
...
- **更新** 工具调用事件系统
  - 定义ToolEvent结构体，支持工具开始、进度、成功、错误四种事件类型
  - 通过sendToolEvent函数发送工具状态事件到流中
  - 支持工具调用的实时进度传输和客户端渲染
- **更新** antsx库模式
...
participant ToolEvent as "工具事件"
participant MCPClient as "MCP客户端"
Client->>ChatUI : "发送聊天请求"
ChatUI->>ChatAPI : "POST /aigtw/v1/chat/completions"
ChatAPI->>ToolEvent : "检测到工具调用"
ToolEvent->>ToolEvent : "发送tool_start事件"
ToolEvent->>MCPClient : "执行工具调用"
MCPClient-->>ToolEvent : "进度回调"
ToolEvent->>ToolEvent : "发送tool_progress事件"
MCPClient-->>ToolEvent : "工具执行完成"
ToolEvent->>ToolEvent : "发送tool_success事件"
ToolEvent-->>ChatUI : "流式传输工具事件"
ChatUI-->>Client : "实时渲染工具进度"
```
...
- [aiapp/aichat/internal/logic/chatcompletionstreamlogic.go:108-276](file://aiapp/aichat/internal/logic/chatcompletionstreamlogic.go#L108-L276)
...
- **更新** PingReply：健康检查返回结构（已移除Ping接口）。
- SSEStreamRequest：包含可选 channel。
- **更新** ToolEvent：工具调用事件结构体，包含事件类型、工具ID、工具名称、索引、进度、消息、结果摘要、错误和持续时间等字段。
...
- **更新** AsyncToolResultRequest：异步工具结果查询请求
- **更新** AsyncToolResultResponse：异步工具结果查询响应

**章节来源**
- [aiapp/ssegtw/internal/types/types.go:6-17](file://aiapp/ssegtw/internal/types/types.go#L6-L17)
- [aiapp/aigtw/internal/types/types.go:14-91](file://aiapp/aigtw/internal/types/types.go#L14-L91)
- [aiapp/aichat/internal/logic/chatcompletionstreamlogic.go:23-35](file://aiapp/aichat/internal/logic/chatcompletionstreamlogic.go#L23-L35)
...
- **客户端集成示例**：
  ...
  - 支持流式和非流式两种模式
  - **更新** 通过 `/aigtw/v1/async-tool-results` 轮询异步工具执行状态

- **JavaScript 功能**：
  - 实时流式处理，支持深度思考标签解析
  - **更新** 工具事件处理：支持tool_start、tool_progress、tool_success、tool_error四种事件类型
  - **更新** 工具卡片渲染：实时显示工具执行进度和状态
  - Markdown 渲染，支持代码块高亮
  - 键盘快捷键支持（Enter 发送）
  - 停止流式输出功能
  - 本地存储主题和语言偏好
...
### 工具事件定义与类型
AI聊天服务新增了完整的工具调用事件系统，支持工具执行过程的实时可视化：

- **ToolEvent结构体**：
  - Type：事件类型（tool_start、tool_progress、tool_success、tool_error）
  - ToolId：工具调用ID
  - ToolName：工具名称
  - Index：工具序号（1-based）
  - Progress：执行进度百分比
  ...
  - ResultSummary：结果摘要
  - Error：错误信息
  - DurationMs：执行持续时间（毫秒）

- **事件类型说明**：
  - tool_start：工具开始执行事件
  - tool_progress：工具执行进度事件
  - tool_success：工具执行成功事件
  - tool_error：工具执行错误事件

### 工具事件发送机制
工具事件通过sendToolEvent函数发送到流中，采用特殊的消息格式：

```mermaid
flowchart TD
Start(["工具调用开始"]) --> Detect["检测到工具调用"]
Detect --> SendStart["发送tool_start事件"]
SendStart --> Execute["执行工具调用"]
Execute --> Progress["工具执行进度回调"]
Progress --> SendProgress["发送tool_progress事件"]
Execute --> Success["工具执行成功"]
Success --> SendSuccess["发送tool_success事件"]
Execute --> Error["工具执行错误"]
Error --> SendError["发送tool_error事件"]
```

**图表来源**
- [aiapp/aichat/internal/logic/chatcompletionstreamlogic.go:201-266](file://aiapp/aichat/internal/logic/chatcompletionstreamlogic.go#L201-L266)

### 客户端工具事件处理
聊天界面新增了完整的工具事件处理逻辑：

- **handleToolEvent函数**：根据事件类型调用相应的处理函数
- **handleToolStart函数**：初始化工具卡片，设置初始状态
- **handleToolProgress函数**：更新工具进度，支持节流渲染
- **handleToolSuccess函数**：标记工具执行成功，显示结果摘要
- **handleToolErrorEvent函数**：标记工具执行错误，显示错误信息

- **工具卡片渲染**：
  - renderToolCards函数：渲染所有工具卡片，支持运行中、成功、错误三种状态
  - 现代卡片样式，包含状态图标、进度条、消息显示
  - 支持按工具序号排序显示
...
- **AsyncToolCallLogic**：异步调用MCP工具，返回任务ID和状态
- **AsyncToolResultLogic**：查询异步工具调用结果，支持轮询
- **异步工具结果处理器**：AI网关服务提供RESTful接口查询异步工具执行状态

**章节来源**
- [aiapp/aichat/internal/logic/asynctoolcalllogic.go:26-70](file://aiapp/aichat/internal/logic/asynctoolcalllogic.go#L26-L70)
- [aiapp/aichat/internal/logic/asynctoolresultlogic.go:24-56](file://aiapp/aichat/internal/logic/asynctoolresultlogic.go#L24-L56)
- [aiapp/aigtw/internal/logic/pass/asynctoolresultlogic.go:31-61](file://aiapp/aigtw/internal/logic/pass/asynctoolresultlogic.go#L31-L61)
...
**InvokeWithReactor**：
```go
reactor, _ := antsx.NewReactor(100)
...
ChatCompletionStreamLogic --> ServiceContext : "依赖"
ChatCompletionStreamLogic --> ToolEvent : "使用"
ToolEvent --> Writer : "通过流发送"
```
...
- [aiapp/aichat/internal/logic/chatcompletionstreamlogic.go:56-67](file://aiapp/aichat/internal/logic/chatcompletionstreamlogic.go#L56-L67)
- [common/ssex/writer.go:14-54](file://common/ssex/writer.go#L14-L54)
- [aiapp/aichat/internal/logic/chatcompletionstreamlogic.go:23-35](file://aiapp/aichat/internal/logic/chatcompletionstreamlogic.go#L23-L35)
...
- **更新** 工具事件性能优化：
  - 工具进度事件支持节流渲染，避免频繁DOM更新
  - 工具卡片状态变化采用批量更新，减少重绘次数
  - 工具错误事件支持自动隐藏，避免界面拥挤
- **更新** antsx库性能优化：
...
- **更新** AI 网关服务的性能优化：
  ...
  - 本地存储：减少重复请求，提升响应速度
  - **更新** 工具事件渲染：采用虚拟DOM更新策略，提升工具卡片渲染性能
...
**更新** AI聊天服务新增的工具调用事件系统显著提升了用户体验：
- 提供完整的工具执行可视化，包括开始、进度、成功、错误四种状态
- 实时进度展示，支持工具执行的透明化监控
- 工具卡片渲染，提供直观的状态反馈
- 节流渲染优化，避免频繁DOM更新影响性能
- 错误处理机制，支持工具执行异常的可视化提示
...
- **AI 网关服务**
  ...
  - **更新** 异步工具结果查询
    - 方法：GET
    - 路径：/aigtw/v1/async-tool-results
    - 请求体：AsyncToolResultRequest
...
- **AI 网关服务**
  ...
  - **更新** 轮询异步工具结果：`/aigtw/v1/async-tool-results?task_id=...`
...
- **AI 网关服务**
  ...
  - **更新** 工具调用客户端：配置MCP客户端，支持工具执行和进度回调
...
- **服务端实现最佳实践**：
  - 工具事件JSON序列化要确保字段完整性
  - 进度回调函数要支持幂等操作，避免重复更新
  - 工具执行错误要提供详细的错误信息和恢复建议
  - 工具结果要进行适当的截断处理，避免消息过大

- **客户端渲染最佳实践**：
  - 工具进度事件要进行节流处理，避免频繁DOM更新
  - 工具卡片状态变化要采用批量更新策略
  - 错误事件要提供用户友好的提示信息
  - 工具执行时间要进行格式化显示，提升可读性

- **性能优化建议**：
  - 工具事件采用虚拟DOM更新策略，减少真实DOM操作
  - 工具卡片渲染支持懒加载，避免一次性渲染过多卡片
  - 工具进度回调采用requestAnimationFrame进行节流
  - 工具错误事件支持自动隐藏，避免界面拥挤

**章节来源**
- [aiapp/aichat/internal/logic/chatcompletionstreamlogic.go:201-266](file://aiapp/aichat/internal/logic/chatcompletionstreamlogic.go#L201-L266)
- [aiapp/aigtw/chat.html:2359-2513](file://aiapp/aigtw/chat.html#L2359-2513)
- [common/mcpx/client.go:77-93](file://common/mcpx/client.go#L77-L93)