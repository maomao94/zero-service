...
- **超时配置管理**：支持SSE连接超时（24小时）和工具执行超时（30分钟）配置，支持更灵活的部署场景
- **上下文传播机制**：实现HTTP头部与上下文的双向映射和自动传播，支持三种认证路径
...
- **日志增强功能**：通过CallToolWrapper提供统一的日志记录机制，提升调试效率
- **响应消息增强**：在响应消息中支持中文用户名显示，提升用户体验
...
└── internal/
    ...
    └── tools/
        ...
        ├── testprogress.go   # testprogress工具实现（使用RunSafe线程机制）
        └── registry.go       # 工具注册中心
```
...
### 工具注册中心
- **RegisterAll函数**：统一注册所有工具，包括testprogress工具
- **工具注册机制**：支持动态工具注册和管理
...
SSE["SSE传输<br/>common/mcpx/server.go"]
...
THREEPATHS["三种认证路径<br/>SSE直连/JWT"]
...
- **配置项**
  ...
  - Mode: dev：开发环境标识
  - Mcp.UseStreamable: false：**更新** 禁用Streamable HTTP传输协议，改用SSE传输
  - Mcp.SseTimeout: 24h：SSE连接超时配置（24小时）
  - Mcp.MessageTimeout: 18000s：**更新** 工具执行超时配置（30分钟），从注释状态恢复启用
...
Loop --> RunSafe["RunSafe线程机制"]
RunSafe --> Sleep["等待间隔时间"]
Sleep --> Loop
Loop --> Complete["完成处理"]
...
### 工具执行超时配置
- **MessageTimeout字段**：**更新** 从注释状态恢复启用的工具执行超时配置，默认30分钟
- **配置位置**：在Mcp配置段落中设置
- **应用场景**：控制工具调用的最大执行时间
- **性能优化**：防止长时间阻塞影响服务器性能
...
### 上下文提取机制
**更新** 三种认证路径的自动上下文提取机制：
1. **SSE传输**：从req.Params._meta提取用户上下文
2. **SSE直连JWT**：从连接级TokenInfo提取上下文
3. **自动传播**：将提取的上下文注入到工具处理器的context中
...
- **JwtSecrets**：支持多个JWT密钥，提高安全性
- **ServiceToken**：连接级服务令牌，用于内部服务间通信
- **ClaimMapping**：**新增** JWT声明映射配置，支持外部JWT claim key到内部标准key的映射
- **配置验证**：支持空配置，无认证时跳过认证中间件

### 传输协议配置
- **UseStreamable**：**更新** 禁用Streamable HTTP传输协议，改用SSE传输
- **MessageTimeout**：**更新** 工具调用消息超时时间（默认30秒，现配置为30分钟）
- **Cors**：允许的跨域来源列表
...
**图表来源**
- [mcpserver.yaml:18-19](file://aiapp/mcpserver/etc/mcpserver.yaml#L18-L19)
- [auth.go:15-48](file://common/mcpx/auth.go#L15-L48)
...
- **传输协议优化**：**更新** 禁用Streamable HTTP协议，改用SSE协议，减少连接开销
- **上下文传播优化**：**更新** 三种认证路径的高效头部映射和上下文注入机制
...
- **日志增强优化**：通过CallToolWrapper提供统一的日志记录机制，提升调试效率
- **响应增强优化**：中文用户名显示提升用户体验，无需额外处理开销
- **CallToolWrapper优化**：统一的工具处理机制，提供更好的性能监控和错误处理
- **上下文提取优化**：**更新** 三种认证路径的自动上下文提取，提升工具调用可靠性
- **进度通知优化**：基于上下文的进度发送器，支持实时进度更新
- **testprogress工具优化**：**更新** 使用RunSafe线程机制替代GoSafe，提高进度测试功能的可靠性
- **超时配置优化**：SSE连接超时（24小时）和工具执行超时（30分钟）配置，支持更灵活的部署场景
...
- **testprogress工具问题**
  ...
  - **updated** 验证默认值处理机制
  - **updated** 检查RunSafe线程机制的可靠性
- **超时配置问题**
  - **updated** 检查SseTimeout配置是否为有效的24小时格式
  - **updated** 验证MessageTimeout配置是否为有效的18000秒格式
  - **updated** 确认超时配置是否正确应用到SSE和POST端点
  - **updated** 验证超时配置对服务器性能的影响
- **三种认证路径问题**
  - **updated** 验证SSE + mcpx.Client路径的上下文传播
  - **updated** 确认SSE直连JWT路径的TokenInfo提取
  - **updated** 验证自动追踪机制的Trace上下文提取
- **进度通知问题**
...
- **现代化工具处理机制**：采用CallToolWrapper统一包装所有工具，提供更好的日志记录、上下文提取和错误处理能力
- **双层认证系统**：支持JWT令牌和连接级服务令牌的双重验证机制
- **SSE传输协议改进**：**更新** 新增的SSE认证处理机制，确保认证上下文的一致性
- **超时配置管理**：**更新** 支持SSE连接超时（24小时）和工具执行超时（30分钟）配置，提供更灵活的部署场景
- **上下文传播机制**：**更新** 实现HTTP头部与上下文的双向映射和自动传播，支持三种认证路径
- **模块化架构**：完全符合go-zero合规布局，便于维护和扩展
- **增强配置管理**：支持Auth配置段落、useStreamable标志和超时配置
- **现代化配置结构**：采用简化的配置层次，提供更好的可读性和维护性
- **优化日志配置**：debug级别日志提供详细信息，同时通过禁用统计日志优化启动性能
- **工具丰富性**：包含基础的echo工具、实用的Modbus工具和testprogress进度通知工具
...
- **日志增强功能**：通过CallToolWrapper提供统一的日志记录机制，提升调试效率
- **响应消息增强**：在响应消息中支持中文用户名显示，提升用户体验
- **日志级别优化**：将日志级别调整为debug，提供更详细的调试信息用于开发和故障排除
- **进度通知功能**：testprogress工具支持实时进度更新，用于演示长时间任务的进度反馈
...
- **详细调试日志**：将日志级别调整为debug，提供更详细的调试信息用于开发和故障排除
- **超时配置优化**：SSE连接超时（24小时）和工具执行超时（30分钟）配置，支持更灵活的部署场景
...
- 实现更精细的工具路由和权限控制
- 增密集成自动化密钥轮换工具
- 扩展CallToolWrapper功能到其他工具类型
- 增强响应消息的国际化支持
- 优化CallToolWrapper的性能监控和错误处理机制
- 利用debug级别日志进行更深入的故障诊断和性能分析
- 实现更完善的进度通知机制和实时反馈功能
- 扩展testprogress工具的功能，支持更多类型的进度通知场景
- 优化超时配置管理，支持动态超时调整和监控
- 增强长时间连接的稳定性和可靠性保障
- 实现三种认证路径的统一上下文传播机制
- 提升线程安全机制的可靠性和性能表现
...
- **Port**：监听端口
- **Mode**：开发环境标识（dev）
- **Mcp.UseStreamable**：**更新** 禁用Streamable HTTP传输协议（false）
- **Mcp.SseTimeout**：SSE连接超时配置（默认24小时）
- **Mcp.MessageTimeout**：**更新** 工具执行超时配置（默认18000秒，现配置为30分钟）
...
- **Auth.ClaimMapping**：**更新** JWT声明映射配置
...
Mode: dev
Mcp:
  UseStreamable: false          # 禁用流式连接，使用SSE传输
  SseTimeout: 24h               # SSE 连接超时
  MessageTimeout: 18000s        # 工具执行超时，默认 30s，改为 30 分钟
Log:
  Encoding: plain
...
- **传输协议选择**：**更新** 根据使用场景选择合适的传输协议，当前配置为SSE传输
- **上下文传播**：**更新** 确保三种认证路径下的上下文传播机制正常工作
...
- **日志增强实践**：利用CallToolWrapper提供的统一日志记录功能提升调试效率
- **响应增强实践**：利用中文用户名显示提升用户体验
- **上下文传播最佳实践**：确保三种认证路径下的上下文提取和传播机制正常工作
- **CallToolWrapper最佳 practice**：正确使用CallToolWrapper包装工具处理器，确保统一的日志记录和错误处理
- **现代化工具处理**：采用CallToolWrapper替代传统的WithCtxProp，提升工具处理的可靠性和可维护性
- **调试日志优化**：利用debug级别日志进行深入的故障诊断和性能分析
- **进度通知最佳实践**：正确使用GetProgressSender获取进度发送器，确保进度通知的准确性和及时性
- **testprogress工具最佳实践**：**更新** 合理设置steps和interval参数，使用RunSafe线程机制确保进度测试的可靠性
- **上下文传播最佳实践**：确保进度发送器的上下文注入和提取机制正常工作
- **超时配置最佳实践**：**updated** 合理配置SseTimeout和MessageTimeout，平衡连接稳定性和性能表现
- **长时间连接最佳实践**：利用24小时SseTimeout配置支持长时间连接场景
