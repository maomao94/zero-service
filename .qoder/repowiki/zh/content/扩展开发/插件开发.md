# 插件开发

<cite>
**本文引用的文件**   
- [README.md](file://README.md)
- [go.mod](file://go.mod)
- [loggerInterceptor.go](file://common/Interceptor/rpcserver/loggerInterceptor.go)
- [metadataInterceptor.go](file://common/Interceptor/rpcclient/metadataInterceptor.go)
- [ctxData.go](file://common/ctxdata/ctxData.go)
- [grpc.go](file://common/ctxprop/grpc.go)
- [http.go](file://common/ctxprop/http.go)
- [claims.go](file://common/ctxprop/claims.go)
- [register.go](file://common/nacosx/register.go)
- [resolver.go](file://common/nacosx/resolver.go)
- [servicecontext.go](file://app/trigger/internal/svc/servicecontext.go)
- [config.go](file://app/trigger/internal/config/config.go)
- [interface.go](file://common/iec104/client/interface.go)
- [handle.go](file://common/iec104/client/handle.go)
- [pending.go](file://common/antsx/pending.go)
- [pending_test.go](file://common/antsx/pending_test.go)
- [rest-api-patterns.md](file://.trae/skills/zero-skills/references/rest-api-patterns.md)
- [client.go](file://common/mcpx/client.go)
- [server.go](file://common/mcpx/server.go)
- [ctxprop.go](file://common/mcpx/ctxprop.go)
</cite>

## 更新摘要
**变更内容**   
- 通信协议与上下文传递机制已从直接操作gRPC元数据改为使用ctxprop包的传播机制
- 新增了统一的上下文传播接口，支持gRPC和HTTP两种传输协议
- 更新了拦截器实现，使用ctxprop.InjectToGrpcMD和ctxprop.ExtractFromGrpcMD函数
- 新增了HTTP头部传播机制，支持MCP协议的上下文传递
- 增强了JWT令牌处理，支持用户侧和服务侧两种认证模式

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [组件详解](#组件详解)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本技术文档面向Zero-Service的插件开发者，系统阐述插件系统的架构设计与开发方法，涵盖插件接口定义、动态加载机制与生命周期管理；解释插件开发的核心概念，包括插件注册、依赖注入、配置管理与事件处理；说明插件与主系统的集成方式，包括服务发现、通信协议与数据交换；给出最佳实践、调试技巧、性能监控与安全考虑，并提供可落地的插件开发示例与测试验证路径。

**更新** 通信协议已升级为基于ctxprop包的统一传播机制，支持gRPC和HTTP两种传输协议的上下文传递。

## 项目结构
Zero-Service采用go-zero微服务框架，围绕gRPC/HTTP聚合、消息队列、任务调度、协议适配与实时通信等能力构建。插件开发应遵循以下组织方式：
- 插件接口定义：在公共模块中抽象插件能力，例如协议适配接口、事件处理接口等
- 插件注册与发现：通过Nacos进行服务注册与解析，实现插件的动态发现与负载均衡
- 依赖注入：在ServiceContext中集中初始化插件所需的共享资源（数据库、缓存、RPC客户端等）
- 生命周期管理：在插件启动时完成初始化，在进程退出时完成清理
- 通信协议：统一使用ctxprop包进行上下文传播，支持gRPC和HTTP协议
- 配置管理：通过配置文件与环境变量驱动插件行为

```mermaid
graph TB
subgraph "插件宿主"
SC["ServiceContext<br/>依赖注入容器"]
CFG["Config<br/>配置对象"]
NACOS_REG["Nacos 注册/发现"]
INT_S["RPC 服务端拦截器<br/>LoggerInterceptor"]
INT_C["RPC 客户端拦截器<br/>UnaryMetadataInterceptor"]
CTX_PROP["ctxprop 包<br/>统一上下文传播"]
end
subgraph "插件实现"
IFACE["插件接口定义<br/>如 IEC104 客户端处理接口"]
HANDLER["插件处理器<br/>如 IEC104 客户端事件处理器"]
EVT["事件/请求等待器<br/>PendingRegistry"]
end
CFG --> SC
NACOS_REG --> SC
INT_S --> CTX_PROP
INT_C --> CTX_PROP
IFACE --> HANDLER
HANDLER --> EVT
SC --> HANDLER
CTX_PROP --> INT_S
CTX_PROP --> INT_C
```

**图表来源**
- [servicecontext.go:29-89](file://app/trigger/internal/svc/servicecontext.go#L29-L89)
- [config.go:9-27](file://app/trigger/internal/config/config.go#L9-L27)
- [register.go:21-76](file://common/nacosx/register.go#L21-L76)
- [loggerInterceptor.go:12-44](file://common/Interceptor/rpcserver/loggerInterceptor.go#L12-L44)
- [metadataInterceptor.go:11-32](file://common/Interceptor/rpcclient/metadataInterceptor.go#L11-L32)
- [grpc.go:11-34](file://common/ctxprop/grpc.go#L11-L34)
- [http.go:10-32](file://common/ctxprop/http.go#L10-L32)
- [interface.go:5-23](file://common/iec104/client/interface.go#L5-L23)
- [handle.go:34-82](file://common/iec104/client/handle.go#L34-L82)
- [pending.go:169-183](file://common/antsx/pending.go#L169-L183)

**章节来源**
- [README.md:1-350](file://README.md#L1-L350)
- [go.mod:1-245](file://go.mod#L1-L245)

## 核心组件
- 服务上下文（ServiceContext）：集中管理数据库、缓存、RPC客户端、任务队列、模型与工具实例，作为插件依赖注入的根容器
- 配置（Config）：承载RPC服务端、注册中心、数据库、Redis、流事件客户端等配置
- 拦截器（LoggerInterceptor、UnaryMetadataInterceptor）：通过ctxprop包在gRPC请求链路中注入与透传上下文信息（用户ID、授权、追踪ID等）
- 服务注册与发现（Nacos）：服务注册、注销与实例变更监听，支持健康检查与负载均衡
- 上下文传播（ctxprop包）：统一的上下文传播机制，支持gRPC元数据和HTTP头部的自动转换
- 插件接口与处理器：以协议适配为例，定义插件接口与事件处理器，结合等待器实现请求-响应配对
- 请求等待器（PendingRegistry）：为请求分配唯一ID并等待响应，支持超时与清理

**更新** 新增了ctxprop包作为统一的上下文传播机制，替代了原有的直接元数据操作方式。

**章节来源**
- [servicecontext.go:29-89](file://app/trigger/internal/svc/servicecontext.go#L29-L89)
- [config.go:9-27](file://app/trigger/internal/config/config.go#L9-L27)
- [loggerInterceptor.go:12-44](file://common/Interceptor/rpcserver/loggerInterceptor.go#L12-L44)
- [metadataInterceptor.go:11-32](file://common/Interceptor/rpcclient/metadataInterceptor.go#L11-L32)
- [grpc.go:11-34](file://common/ctxprop/grpc.go#L11-L34)
- [http.go:10-32](file://common/ctxprop/http.go#L10-L32)
- [claims.go:10-43](file://common/ctxprop/claims.go#L10-L43)
- [register.go:21-76](file://common/nacosx/register.go#L21-L76)
- [resolver.go:38-65](file://common/nacosx/resolver.go#L38-L65)
- [interface.go:5-23](file://common/iec104/client/interface.go#L5-L23)
- [handle.go:34-82](file://common/iec104/client/handle.go#L34-L82)
- [pending.go:169-183](file://common/antsx/pending.go#L169-L183)

## 架构总览
插件系统围绕"接口抽象—注册发现—依赖注入—生命周期管理—统一上下文传播"展开，形成松耦合、可扩展的插件生态。

```mermaid
sequenceDiagram
participant Host as "宿主应用"
participant SC as "ServiceContext"
participant Nacos as "Nacos"
participant Plugin as "插件实现"
participant RPC as "gRPC 服务"
participant CTX as "ctxprop 包"
Host->>SC : 初始化配置与依赖
SC->>Nacos : 注册服务实例
Note over Nacos : 健康检查与实例变更监听
Host->>Plugin : 加载插件接口与处理器
Plugin->>SC : 获取共享依赖DB/Redis/RPC
Plugin->>CTX : 使用ctxprop进行上下文传播
CTX->>RPC : 通过UnaryMetadataInterceptor传播上下文
RPC-->>Plugin : 返回响应
Host->>SC : 进程退出时触发清理
SC->>Nacos : 注销服务实例
```

**更新** 架构图中新增了ctxprop包作为统一的上下文传播层。

**图表来源**
- [servicecontext.go:50-89](file://app/trigger/internal/svc/servicecontext.go#L50-L89)
- [register.go:21-76](file://common/nacosx/register.go#L21-L76)
- [resolver.go:38-65](file://common/nacosx/resolver.go#L38-L65)
- [loggerInterceptor.go:12-44](file://common/Interceptor/rpcserver/loggerInterceptor.go#L12-L44)
- [metadataInterceptor.go:11-32](file://common/Interceptor/rpcclient/metadataInterceptor.go#L11-L32)
- [grpc.go:11-34](file://common/ctxprop/grpc.go#L11-L34)

## 组件详解

### 插件接口定义与事件处理
- 插件接口：以IEC104客户端为例，定义ASDUCall接口，提供多种ASDU事件回调方法，便于插件按需实现
- 事件处理器：ClientHandler封装各类ASDU事件处理，统一记录耗时指标，调用插件接口实现
- 默认空实现：避免空指针，允许插件仅覆盖所需回调

```mermaid
classDiagram
class ASDUCall {
+OnInterrogation(asdu) error
+OnCounterInterrogation(asdu) error
+OnRead(asdu) error
+OnTestCommand(asdu) error
+OnClockSync(asdu) error
+OnResetProcess(asdu) error
+OnDelayAcquisition(asdu) error
+OnASDU(asdu) error
}
class emptyASDUCall {
+OnInterrogation(asdu) error
+OnCounterInterrogation(asdu) error
+OnRead(asdu) error
+OnTestCommand(asdu) error
+OnClockSync(asdu) error
+OnResetProcess(asdu) error
+OnDelayAcquisition(asdu) error
+OnASDU(asdu) error
}
class ClientHandler {
-call ASDUCall
-metrics Metrics
+InterrogationHandler(...)
+CounterInterrogationHandler(...)
+ReadHandler(...)
+TestCommandHandler(...)
+ClockSyncHandler(...)
}
ASDUCall <|.. emptyASDUCall : "默认实现"
ClientHandler --> ASDUCall : "委托调用"
```

**图表来源**
- [interface.go:5-55](file://common/iec104/client/interface.go#L5-L55)
- [handle.go:34-82](file://common/iec104/client/handle.go#L34-L82)

**章节来源**
- [interface.go:5-55](file://common/iec104/client/interface.go#L5-L55)
- [handle.go:34-82](file://common/iec104/client/handle.go#L34-L82)

### 依赖注入与服务上下文
- ServiceContext集中管理所有共享资源：数据库连接、SQL查询构建器、Redis、任务队列客户端、模型、HTTP客户端、流事件RPC客户端等
- 通过NewServiceContext按配置初始化，确保插件在运行时可直接获取所需依赖
- 可扩展：新增插件只需在ServiceContext中添加对应字段并在初始化时赋值

```mermaid
classDiagram
class ServiceContext {
+Config Config
+Validate *validator.Validate
+AsynqClient *asynq.Client
+AsynqInspector *asynq.Inspector
+AsynqServer *asynq.Server
+Scheduler *asynq.Scheduler
+Httpc httpc.Service
+ConnMap *SafeMap
+SqlConn sqlx.SqlConn
+PlanModel PlanModel
+PlanBatchModel PlanBatchModel
+PlanExecItemModel PlanExecItemModel
+PlanExecLogModel PlanExecLogModel
+Database *goqu.Database
+UnstableExpiry Unstable
+Redis *redis.Redis
+StreamEventCli StreamEventClient
+IdUtil *IdUtil
}
```

**图表来源**
- [servicecontext.go:29-48](file://app/trigger/internal/svc/servicecontext.go#L29-L48)

**章节来源**
- [servicecontext.go:29-89](file://app/trigger/internal/svc/servicecontext.go#L29-L89)

### 配置管理
- Config定义了RPC服务端配置、Nacos注册配置、数据库连接串、RedisDB索引、禁用SQL语句日志、优雅停机时长以及流事件RPC客户端配置
- 插件通过Config读取注册中心与下游服务地址，实现动态配置驱动

```mermaid
flowchart TD
Start(["读取配置"]) --> Parse["解析配置项<br/>RPC/Nacos/DB/Redis/StreamEvent"]
Parse --> Validate{"配置有效?"}
Validate --> |否| Fail["返回错误并终止启动"]
Validate --> |是| Init["初始化依赖<br/>DB/Redis/RPC/模型"]
Init --> Ready(["插件可用"])
```

**图表来源**
- [config.go:9-27](file://app/trigger/internal/config/config.go#L9-L27)

**章节来源**
- [config.go:9-27](file://app/trigger/internal/config/config.go#L9-L27)

### 服务注册与发现
- 注册：服务启动时向Nacos注册实例，包含权重、健康状态、集群与分组信息，并在进程退出时自动注销
- 解析：监听实例变更，提取健康gRPC实例，排序后更新连接池，保证负载均衡与高可用

```mermaid
sequenceDiagram
participant Svc as "服务实例"
participant Reg as "Nacos 注册"
participant Watch as "实例监听"
participant Conn as "gRPC 连接"
Svc->>Reg : RegisterInstance(服务名/地址/权重/元数据)
Note over Reg : 注册成功后注册注销钩子
Watch->>Reg : 订阅实例变更
Reg-->>Watch : 返回健康实例列表
Watch->>Conn : 更新地址列表(排序去重)
Note over Conn : 负载均衡生效
```

**图表来源**
- [register.go:21-76](file://common/nacosx/register.go#L21-L76)
- [resolver.go:38-65](file://common/nacosx/resolver.go#L38-L65)

**章节来源**
- [register.go:21-76](file://common/nacosx/register.go#L21-L76)
- [resolver.go:38-65](file://common/nacosx/resolver.go#L38-L65)

### 统一上下文传播机制
**更新** Zero-Service现已采用ctxprop包作为统一的上下文传播机制，替代了原有的直接gRPC元数据操作方式。

- ctxprop包：提供统一的上下文传播接口，支持gRPC和HTTP两种传输协议
- gRPC传播：InjectToGrpcMD将context中的字段注入到gRPC元数据，ExtractFromGrpcMD从gRPC元数据恢复到context
- HTTP传播：InjectToHTTPHeader将context中的字段注入到HTTP头部，ExtractFromHTTPHeader从HTTP头部恢复到context
- JWT处理：ExtractFromClaims从JWT令牌中提取用户信息，自动处理数值类型的转换

```mermaid
sequenceDiagram
participant C as "客户端"
participant IC as "UnaryMetadataInterceptor"
participant CTX as "ctxprop 包"
participant S as "服务端"
participant LC as "LoggerInterceptor"
C->>IC : 发起RPC调用(携带上下文)
IC->>CTX : 调用InjectToGrpcMD
CTX-->>C : 注入上下文到gRPC元数据
C->>S : 发送请求
S->>LC : 进入服务端拦截器
LC->>CTX : 调用ExtractFromGrpcMD
CTX-->>S : 从元数据恢复上下文
S-->>C : 返回响应
```

**图表来源**
- [metadataInterceptor.go:11-32](file://common/Interceptor/rpcclient/metadataInterceptor.go#L11-L32)
- [loggerInterceptor.go:12-44](file://common/Interceptor/rpcserver/loggerInterceptor.go#L12-L44)
- [grpc.go:11-34](file://common/ctxprop/grpc.go#L11-L34)
- [ctxData.go:30-38](file://common/ctxdata/ctxData.go#L30-L38)

**章节来源**
- [metadataInterceptor.go:11-32](file://common/Interceptor/rpcclient/metadataInterceptor.go#L11-L32)
- [loggerInterceptor.go:12-44](file://common/Interceptor/rpcserver/loggerInterceptor.go#L12-L44)
- [grpc.go:11-34](file://common/ctxprop/grpc.go#L11-L34)
- [http.go:10-32](file://common/ctxprop/http.go#L10-L32)
- [claims.go:10-43](file://common/ctxprop/claims.go#L10-L43)
- [ctxData.go:30-38](file://common/ctxdata/ctxData.go#L30-L38)

### 生命周期管理
- 初始化：在ServiceContext中完成依赖初始化；插件在启动阶段注册自身接口与处理器
- 运行时：通过ctxprop包与拦截器保持上下文一致性；通过PendingRegistry实现请求-响应配对与超时控制
- 卸载清理：利用进程关闭钩子触发Nacos注销与资源释放

```mermaid
stateDiagram-v2
[*] --> 初始化
初始化 --> 运行中 : "依赖注入/注册服务"
运行中 --> 正常停止 : "优雅停机"
运行中 --> 异常停止 : "错误/中断"
正常停止 --> 清理 : "注销服务/释放资源"
异常停止 --> 清理 : "强制清理"
清理 --> [*]
```

**图表来源**
- [servicecontext.go:50-89](file://app/trigger/internal/svc/servicecontext.go#L50-L89)
- [register.go:59-73](file://common/nacosx/register.go#L59-L73)
- [pending.go:169-183](file://common/antsx/pending.go#L169-L183)

**章节来源**
- [servicecontext.go:50-89](file://app/trigger/internal/svc/servicecontext.go#L50-L89)
- [register.go:59-73](file://common/nacosx/register.go#L59-L73)
- [pending.go:169-183](file://common/antsx/pending.go#L169-L183)

### 插件开发最佳实践
- 接口设计原则：以最小职责接口为核心，提供默认空实现，避免强耦合
- 依赖注入：集中在ServiceContext中初始化，插件只读取不重复初始化
- 配置管理：通过Config统一读取，支持环境变量与配置文件
- 上下文传播：使用ctxprop包进行统一的上下文传播，支持gRPC和HTTP协议
- 事件处理：使用PendingRegistry实现请求-响应配对，支持超时与清理
- 错误处理：在服务端拦截器中统一记录错误日志，便于追踪
- 性能优化：统一指标埋点（如ASDU事件耗时），避免阻塞操作，合理使用连接池与缓存

**更新** 新增了使用ctxprop包进行上下文传播的最佳实践。

**章节来源**
- [interface.go:5-55](file://common/iec104/client/interface.go#L5-L55)
- [handle.go:34-82](file://common/iec104/client/handle.go#L34-L82)
- [pending.go:169-183](file://common/antsx/pending.go#L169-L183)
- [loggerInterceptor.go:12-44](file://common/Interceptor/rpcserver/loggerInterceptor.go#L12-L44)
- [grpc.go:11-34](file://common/ctxprop/grpc.go#L11-L34)
- [http.go:10-32](file://common/ctxprop/http.go#L10-L32)
- [rest-api-patterns.md:382-419](file://.trae/skills/zero-skills/references/rest-api-patterns.md#L382-L419)

## 依赖关系分析
- 插件实现依赖ServiceContext提供的共享资源
- ServiceContext依赖配置与外部中间件（数据库、Redis、RPC客户端）
- Nacos提供服务注册与解析，支撑插件的动态发现与负载均衡
- ctxprop包贯穿gRPC和HTTP全链路，保障上下文与追踪的一致性

```mermaid
graph LR
Plugin["插件实现"] --> SC["ServiceContext"]
SC --> CFG["Config"]
SC --> DB["数据库/Redis"]
SC --> RPC["RPC 客户端"]
SC --> Nacos["Nacos"]
RPC --> InterC["UnaryMetadataInterceptor"]
RPC --> InterS["LoggerInterceptor"]
InterC --> CTX["ctxprop 包"]
InterS --> CTX
CTX --> GRPC["gRPC 元数据传播"]
CTX --> HTTP["HTTP 头部传播"]
```

**更新** 依赖关系图中新增了ctxprop包及其与gRPC和HTTP传播的关系。

**图表来源**
- [servicecontext.go:29-89](file://app/trigger/internal/svc/servicecontext.go#L29-L89)
- [config.go:9-27](file://app/trigger/internal/config/config.go#L9-L27)
- [register.go:21-76](file://common/nacosx/register.go#L21-L76)
- [metadataInterceptor.go:11-32](file://common/Interceptor/rpcclient/metadataInterceptor.go#L11-L32)
- [loggerInterceptor.go:12-44](file://common/Interceptor/rpcserver/loggerInterceptor.go#L12-L44)
- [grpc.go:11-34](file://common/ctxprop/grpc.go#L11-L34)
- [http.go:10-32](file://common/ctxprop/http.go#L10-L32)

**章节来源**
- [servicecontext.go:29-89](file://app/trigger/internal/svc/servicecontext.go#L29-L89)
- [config.go:9-27](file://app/trigger/internal/config/config.go#L9-L27)
- [register.go:21-76](file://common/nacosx/register.go#L21-L76)
- [metadataInterceptor.go:11-32](file://common/Interceptor/rpcclient/metadataInterceptor.go#L11-L32)
- [loggerInterceptor.go:12-44](file://common/Interceptor/rpcserver/loggerInterceptor.go#L12-L44)
- [grpc.go:11-34](file://common/ctxprop/grpc.go#L11-L34)
- [http.go:10-32](file://common/ctxprop/http.go#L10-L32)

## 性能考量
- 指标埋点：在事件处理器中记录耗时，便于定位慢调用
- 连接池与缓存：统一在ServiceContext中初始化，减少插件重复创建
- 超时控制：PendingRegistry支持默认TTL与覆盖TTL，避免悬挂请求
- gRPC配置：设置最大消息大小与拦截器，提升吞吐与稳定性
- 上下文传播优化：ctxprop包通过PropFields统一管理传播字段，减少重复操作

**更新** 新增了ctxprop包在上下文传播方面的性能优化考量。

**章节来源**
- [handle.go:34-82](file://common/iec104/client/handle.go#L34-L82)
- [pending.go:169-183](file://common/antsx/pending.go#L169-L183)
- [servicecontext.go:79-87](file://app/trigger/internal/svc/servicecontext.go#L79-L87)
- [grpc.go:11-34](file://common/ctxprop/grpc.go#L11-L34)

## 故障排查指南
- 错误日志：服务端拦截器统一记录错误，便于快速定位
- 超时问题：检查PendingRegistry的默认TTL与覆盖TTL，确认sendFn是否成功发送
- 注册异常：确认Nacos注册参数（服务名、集群、分组、权重、健康状态）
- 上下文缺失：检查UnaryMetadataInterceptor和LoggerInterceptor是否正确调用ctxprop函数
- 字段传播问题：确认ctxdata.PropFields中定义的字段是否正确配置
- JWT认证问题：检查ExtractFromClaims函数是否正确处理JWT令牌

**更新** 新增了基于ctxprop包的故障排查指导。

**章节来源**
- [loggerInterceptor.go:40-42](file://common/Interceptor/rpcserver/loggerInterceptor.go#L40-L42)
- [pending_test.go:314-342](file://common/antsx/pending_test.go#L314-L342)
- [register.go:40-73](file://common/nacosx/register.go#L40-L73)
- [metadataInterceptor.go:11-32](file://common/Interceptor/rpcclient/metadataInterceptor.go#L11-L32)
- [grpc.go:11-34](file://common/ctxprop/grpc.go#L11-L34)
- [claims.go:10-43](file://common/ctxprop/claims.go#L10-L43)

## 结论
Zero-Service的插件体系以go-zero为基础，通过清晰的接口抽象、完善的依赖注入、统一的ctxprop包上下文传播机制与服务发现机制，实现了插件的可插拔与可扩展。新的ctxprop包提供了更加统一和可靠的上下文传播方式，支持gRPC和HTTP两种传输协议，增强了系统的安全性与可维护性。遵循本文的最佳实践与流程，开发者可快速构建稳定、可观测、易维护的插件。

## 附录

### 插件开发完整示例（步骤说明）
- 定义插件接口：参考ASDUCall接口，按需声明回调方法
- 实现插件处理器：参考ClientHandler，封装事件处理并记录耗时
- 注册与注入：在ServiceContext中添加插件所需依赖，并在初始化时完成注册
- 配置与启动：通过Config读取Nacos与下游服务配置，启动服务并注册到Nacos
- 上下文传播：使用ctxprop包进行统一的上下文传播，确保gRPC和HTTP协议的一致性
- 事件等待：使用PendingRegistry实现请求-响应配对，设置合理TTL
- 测试验证：编写单元测试，验证超时、覆盖TTL与错误路径

**更新** 新增了使用ctxprop包进行上下文传播的开发步骤。

**章节来源**
- [interface.go:5-55](file://common/iec104/client/interface.go#L5-L55)
- [handle.go:34-82](file://common/iec104/client/handle.go#L34-L82)
- [servicecontext.go:50-89](file://app/trigger/internal/svc/servicecontext.go#L50-L89)
- [config.go:9-27](file://app/trigger/internal/config/config.go#L9-L27)
- [register.go:21-76](file://common/nacosx/register.go#L21-L76)
- [grpc.go:11-34](file://common/ctxprop/grpc.go#L11-L34)
- [http.go:10-32](file://common/ctxprop/http.go#L10-L32)
- [pending.go:169-183](file://common/antsx/pending.go#L169-L183)
- [pending_test.go:314-342](file://common/antsx/pending_test.go#L314-L342)