...
- **重要变更**：移除了专用的trace ID处理，改为使用OpenTelemetry标准的traceparent和tracestate进行分布式追踪
...
- 用户ID、用户名、部门编码、授权信息的提取函数与使用方式
- **更新**：分布式追踪机制采用OpenTelemetry标准的traceparent和tracestate头部
...
**图表来源**
- [common/ctxdata/ctxData.go:1-67](file://common/ctxdata/ctxData.go#L1-L67)
- [common/ctxprop/grpc.go:1-35](file://common/ctxprop/grpc.go#L1-L35)
- [common/ctxprop/http.go:1-45](file://common/ctxprop/http.go#L1-L45)
- [common/ctxprop/claims.go:1-69](file://common/ctxprop/claims.go#L1-L69)
...
- **传播框架**
  ...
  - InjectToHTTPHeader/ExtractFromHTTPHeader：HTTP头部传播，**新增**：自动处理traceparent和tracestate追踪头部
...
- **提取函数**
  - GetUserId、GetUserName、GetAuthorization、GetDeptCode：从 context.Context 中读取对应字符串值；不存在则返回空串
  - **注意**：不再提供GetTraceId函数，因为追踪ID现在使用OpenTelemetry标准头部
- **拦截器机制**
...
- [common/ctxprop/http.go:10-44](file://common/ctxprop/http.go#L10-L44)
- [common/ctxprop/claims.go:10-69](file://common/ctxprop/claims.go#L10-L69)

## 架构总览
上下文数据在多传输层中的流转路径：
- HTTP/WS 层：在进入业务逻辑前，将JWT或会话信息解析并写入context.Context
- 传输层拦截器：根据PropFields配置，自动在不同传输层间转换上下文数据
- **更新**：HTTP传播机制现在自动处理OpenTelemetry标准的traceparent和tracestate头部
- gRPC 客户端拦截器：从context.Context读取上下文数据，注入到outgoing metadata
- gRPC 服务端拦截器：从incoming metadata读取并回写到context.Context
- 业务逻辑层：通过提取函数读取上下文数据，用于鉴权、审计与跨服务透传
...
InterC->>InterC : "自动处理traceparent/tracestate"
...
- [common/ctxprop/http.go:12-44](file://common/ctxprop/http.go#L12-L44)
- [common/ctxprop/claims.go:13-69](file://common/ctxprop/claims.go#L13-L69)
...
- **PropFields统一定义**
  ...
  - **更新**：不再包含trace-id字段，追踪现在使用OpenTelemetry标准头部
- **PropField结构详解**
...
+traceparent : "OpenTelemetry追踪头部"
+tracestate : "OpenTelemetry追踪状态头部"
+自动处理 : "HTTP传播机制"
}
...
- [common/msgbody/msgbody.go:5-19](file://common/msgbody/msgbody.go#L5-L19)
- [common/ctxprop/claims.go:13-69](file://common/ctxprop/claims.go#L13-L69)
- [common/ctxprop/http.go:18-23](file://common/ctxprop/http.go#L18-L23)
...
- **HTTP传播机制**
  ...
  - **更新**：自动处理OpenTelemetry标准的traceparent和tracestate头部
  - 支持标准HTTP头部格式
- **JWT Claims转换**
...
SetHdr --> CheckTrace{"检查traceparent/tracestate"}
CheckTrace --> |存在| SetTrace["设置追踪头部"]
CheckTrace --> |不存在| SkipTrace["跳过"]
SetTrace --> WriteHdr["写回context"]
SkipTrace --> WriteHdr
WriteHdr --> End
...
- **追踪头部处理**
  - HTTP传播机制自动处理traceparent和tracestate头部
  - 与标准OpenTelemetry规范兼容
...
ISrv --> MD
Trace["追踪传播"] --> HTTP["HTTP头部传播"]
Trace --> GRPC["gRPC元数据传播"]
```
...
- **传播机制优化**
  ...
  - **更新**：HTTP传播机制对traceparent/tracestate头部的处理是条件性的，只有当存在时才进行处理
- **拦截器性能**
...
- **症状：追踪链路中断**
  - **更新**：确认HTTP传播机制是否正确处理traceparent和tracestate头部
  - 检查上游服务是否正确设置OpenTelemetry追踪头部
  - 验证下游服务是否正确提取追踪头部
  - 确认OpenTelemetry配置是否正确
- **症状：消息体传播异常**
...
## 结论
Zero-Service通过重构的上下文数据管理机制，实现了多传输层的统一上下文传播。新的PropFields机制简化了字段定义和传播逻辑，ctxprop包提供了统一的传播框架，消息体结构支持OpenTelemetry集成，拦截器机制提供了自动化的上下文传播。JWT Claims转换机制进一步增强了系统的易用性。**重要更新**：分布式追踪现在采用OpenTelemetry标准的traceparent和tracestate头部，提供了更好的标准化和互操作性。这种设计既保证了系统的可扩展性，又确保了跨服务调用时上下文数据的一致性和完整性。建议在所有涉及跨服务调用的场景中统一使用该模式，并充分利用PropFields机制来管理上下文字段。

## 附录

### 常量与键映射速查
- **上下文键**
  - user-id、user-name、dept-code、authorization、auth-type
  - **注意**：不再包含trace-id键，追踪现在使用OpenTelemetry标准头部
- **gRPC头部键**
  - x-user-id、x-user-name、x-dept-code、authorization、x-auth-type
  - **注意**：不再包含x-trace-id头部
- **HTTP头部键**
  - X-User-Id、X-User-Name、X-Dept-Code、Authorization、X-Auth-Type
  - **新增**：traceparent、tracestate（OpenTelemetry标准追踪头部）
- **数据结构字段**
...
- **追踪头部**
  - traceparent：OpenTelemetry标准追踪父级标识
  - tracestate：OpenTelemetry标准追踪状态信息
