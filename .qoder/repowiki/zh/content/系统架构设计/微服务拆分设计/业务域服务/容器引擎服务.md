...
## 核心组件
- gRPC 服务定义：在 podengine.proto 中定义了 PodEngine 服务及消息类型，涵盖 Pod 生命周期、容器状态、资源统计、镜像列表等。
...
- Pod/Container 模型：包含 PodPhase、PodCondition、ContainerState、Container、ContainerSpec、PodSpec、Pod、ContainerStats、Image 等消息类型，覆盖生命周期、状态、资源统计与镜像信息。
- 参数校验：使用 validate 规则对关键字段进行约束，如 PodSpec.containers 非空、restartPolicy 限定值、ListPodsReq/ ListImagesReq 的 limit/offset 上限等。
...
PodEngine --> PodSpec
PodEngine --> ContainerSpec
PodEngine --> Pod
PodEngine --> Container
PodEngine --> ContainerStats
PodEngine --> Image
Pod --> Container
...
#### 创建容器（CreatePod）
- 输入校验：验证请求体与 PodSpec，确保至少包含一个容器。
- Docker 客户端选择：根据 node 获取对应 Docker 客户端（默认 local）。
- 配置构建：将 ContainerSpec 映射为 container.Config，设置镜像、环境变量、命令、标签等；HostConfig 设置端口绑定、重启策略、网络模式、资源限制、卷挂载等；NetworkConfig 为空。
- 资源解析：解析 CPU/内存限额与请求，转换为 Docker 资源对象；解析端口映射与卷挂载格式。
- 创建与检查：调用 ContainerCreate 创建容器，随后 ContainerInspect 获取创建后的容器信息，组装 Pod 返回。

```mermaid
flowchart TD
Start(["开始: CreatePod"]) --> Validate["校验请求与 PodSpec"]
...
- 检查与组装：ContainerInspect 获取最新状态，组装 Pod 返回。
...
- 列表：支持按 id/name/labels 过滤，分页返回容器列表项，包含状态、端口、挂载、大小等信息。
- 查询：按 id 获取单个 Pod 详情。
...
CalcCPU --> Build["组装 ContainerStats"]
CalcMem --> Build
SumNet --> Build
SumBlk --> Build
...
- 端口冲突：创建容器时若端口映射冲突，Docker 会报错。检查 ContainerSpec.ports 格式与宿主机端口占用情况。
