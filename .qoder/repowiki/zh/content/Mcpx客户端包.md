...
## 更新摘要
**变更内容**
- **配置系统重构**：WithOptions函数简化了配置选项处理，直接使用SDK的*mcp.ClientOptions
- **进度系统优化**：ProgressSender结构体从累积进度改为浮点进度值，移除了finished字段和finishedMu互斥锁
- **工具执行生命周期简化**：CallToolWrapper中的Stop方法替代了Done方法，保持向后兼容性
- **包装器函数更新**：ProgressSender API保持向后兼容，提供更简洁的API设计
...
**图表来源**
- [client.go:1-1240](file://common/mcpx/client.go#L1-L1240)
...
**图表来源**
- [wrapper.go:34-48](file://common/mcpx/wrapper.go#L34-L48)
- [wrapper.go:121-129](file://common/mcpx/wrapper.go#L121-L129)
...
**图表来源**
- [wrapper.go:276-277](file://common/mcpx/wrapper.go#L276-L277)
- [wrapper.go:121-129](file://common/mcpx/wrapper.go#L121-L129)
...
**图表来源**
- [wrapper.go:131-153](file://common/mcpx/wrapper.go#L131-L153)
...
- **配置选项简化**: WithOptions函数直接使用SDK的*mcp.ClientOptions，简化配置处理
...
5. **配置选项处理**: WithOptions函数现在直接使用SDK的*mcp.ClientOptions
...
**更新** 最重要的改进是ProgressSender API的优化：Stop方法替代了Done方法，移除了finished字段和finishedMu互斥锁，使API更加简洁和语义化，同时保持了完全的向后兼容性和相同的功能行为。此外，客户端侧进度处理现在使用中文完成消息"异步任务完成"，确保与应用程序整体国际化策略的一致性。配置系统也得到了简化，WithOptions函数现在直接使用SDK的*mcp.ClientOptions，提供了更直观的配置选项处理方式。