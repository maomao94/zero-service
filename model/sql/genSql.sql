...
CREATE TABLE IF NOT EXISTS `plan` (
    `id` VARCHAR(64) NOT NULL COMMENT 'UUID主键ID',
    `create_time` DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT '创建时间',
    `update_time` DATETIME(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6) COMMENT '更新时间',
    `delete_time` DATETIME(6) NULL DEFAULT NULL COMMENT '删除时间（软删除标记）',
    `is_deleted` TINYINT NOT NULL DEFAULT 0 COMMENT '删除状态：0-未删除，1-已删除',
    ...
    `recurrence_rule` TEXT NOT NULL COMMENT '重复规则，JSON格式存储',
    ...
    `scan_flg` INT NOT NULL DEFAULT 0 COMMENT '扫表标记, 0-未扫表, 1-已扫表',
    `terminated_reason` VARCHAR(2000) DEFAULT '' COMMENT '终止原因',
    ...
    `description` VARCHAR(256) NOT NULL DEFAULT '' COMMENT '备注信息',
    `ext_1` VARCHAR(256) DEFAULT '' COMMENT '扩展字段1',
    `ext_2` VARCHAR(256) DEFAULT '' COMMENT '扩展字段2',
    `ext_3` VARCHAR(256) DEFAULT '' COMMENT '扩展字段3',
    `ext_4` VARCHAR(256) DEFAULT '' COMMENT '扩展字段4',
    `ext_5` VARCHAR(256) DEFAULT '' COMMENT '扩展字段5',
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `plan_exec_item` (
    ...
    `update_user` VARCHAR(64) DEFAULT '' COMMENT '更新人',
    `dept_code` VARCHAR(64) DEFAULT '' COMMENT '机构code',
    `plan_pk` VARCHAR(64) NOT NULL DEFAULT '' COMMENT '关联的计划主键ID',
    `plan_id` VARCHAR(64) NOT NULL DEFAULT '' COMMENT '关联的计划ID',
    `batch_pk` VARCHAR(64) NOT NULL DEFAULT '' COMMENT '批主键ID',
    ...
    `last_result` varchar(256) DEFAULT '' COMMENT '上次执行结果',
    `last_message` varchar(2000) DEFAULT '' COMMENT '上次结果描述',
    `last_reason` varchar(2000) DEFAULT '' COMMENT '上次结果原因',
    `terminated_reason` VARCHAR(2000) DEFAULT '' COMMENT '终止原因',
    `paused_time` DATETIME(6) DEFAULT NULL COMMENT '暂停时间',
    `paused_reason` VARCHAR(256) DEFAULT '' COMMENT '暂停原因',
    `ext_1` VARCHAR(256) DEFAULT '' COMMENT '扩展字段1',
    ...
    KEY `idx_status` (`status`),
    KEY `idx_core_scan` (`is_deleted`, `next_trigger_time`, `status`)
...
CREATE TABLE IF NOT EXISTS `plan_exec_log` (
    ...
    `batch_pk` VARCHAR(64) NOT NULL DEFAULT '' COMMENT '批主键ID',
    `batch_id` VARCHAR(64) NOT NULL DEFAULT '' COMMENT '批ID',
    `item_pk` VARCHAR(64) NOT NULL DEFAULT '' COMMENT '关联的执行项主键ID',
    `exec_id` VARCHAR(64) NOT NULL DEFAULT '' COMMENT '执行ID',
    `item_id` VARCHAR(64) NOT NULL DEFAULT '' COMMENT '执行项ID',
    `item_type` VARCHAR(64) DEFAULT '' COMMENT '执行项类型',
    `item_name` VARCHAR(128) NOT NULL DEFAULT '' COMMENT '执行项名称',
    ...
    `reason` varchar(2000) DEFAULT '' COMMENT '结果原因',
    PRIMARY KEY (`id`),
    KEY `idx_plan_pk` (`plan_pk`),
    KEY `idx_plan_id` (`plan_id`),
    KEY `idx_batch_pk` (`batch_pk`),
