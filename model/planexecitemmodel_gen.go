...
type (
	planExecItemModel interface {
		...
		FindOneByItemId(ctx context.Context, itemId string) (*PlanExecItem, error)
...
func (m *defaultPlanExecItemModel) FindOneByItemId(ctx context.Context, itemId string) (*PlanExecItem, error) {
	var resp PlanExecItem
	query := fmt.Sprintf("select %s from %s where `item_id` = ?  and del_state = ? limit 1", planExecItemRows, m.table)
	err := m.conn.QueryRowCtx(ctx, &resp, query, itemId, 0)
	switch err {
	case nil:
		return &resp, nil
	case sqlx.ErrNotFound:
...
func (m *defaultPlanExecItemModel) Insert(ctx context.Context, session sqlx.Session, data *PlanExecItem) (sql.Result, error) {
	...
	if session != nil {
		return session.ExecCtx(ctx, query, data.DeleteTime, data.DelState, data.Version, data.PlanId, data.ItemId, data.ItemName, data.PointId, data.ServiceAddr, data.Method, data.Payload, data.RequestTimeout, data.PlanTriggerTime, data.NextTriggerTime, data.LastTriggerTime, data.TriggerCount, data.Status, data.LastResult, data.LastMsg, data.IsTerminated, data.TerminatedTime, data.TerminatedReason, data.IsPaused, data.PausedTime, data.PausedReason)
	}
...
func (m *defaultPlanExecItemModel) Update(ctx context.Context, session sqlx.Session, newData *PlanExecItem) (sql.Result, error) {
	...
	if session != nil {
		return session.ExecCtx(ctx, query, newData.DeleteTime, newData.DelState, newData.Version, newData.PlanId, newData.ItemId, newData.ItemName, newData.PointId, newData.ServiceAddr, newData.Method, newData.Payload, newData.RequestTimeout, newData.PlanTriggerTime, newData.NextTriggerTime, newData.LastTriggerTime, newData.TriggerCount, newData.Status, newData.LastResult, newData.LastMsg, newData.IsTerminated, newData.TerminatedTime, newData.TerminatedReason, newData.IsPaused, newData.PausedTime, newData.PausedReason, newData.Id)
	}
