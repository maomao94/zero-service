...
| KILOWATT | 千瓦 | kW | 有功功率 |
| HERTZ | 频率 | Hz | 电网频率 |
| DEGREE_CELSIUS | 摄氏温度 | ℃ | 变压器温度 |
| PERCENT | 百分比 | % | 负载率、电池电量等 |
| SECOND | 时间（秒） | s | 状态持续时长 |
| BOOLEAN | 布尔值 | 0/1 | 某些模拟量设备状态等 |
| NONE | 无单位 | - | 特殊情况 |
...
| 枚举值 | 中文名称 | 符号 | 示例用途 |
|--------|----------|------|----------|
| KWH | 电能（千瓦时） | kWh | 电表用电量 |
| MWH | 电能（兆瓦时） | MWh | 发电厂能量输出 |
| M3 | 累计体积 | m³ | 水表、气体表等 |
| LITER | 液体体积 | L | 出水总量 |
| COUNT | 次数 | 次 | 合闸次数、告警次数等 |
| SECOND | 累计时间 | s | 累计运行时间等 |
| NONE | 无单位 | - | 特殊用途 |

#### 🟢 遥信单位（YxUnit）

| 枚举值 | 中文名称 | 前端展示示例 | 示例用途 |
|--------|----------|--------------|----------|
| BOOLEAN | 通用布尔值 | 开 / 关 | 灯光、继电器状态 |
| SWITCH_STATUS | 合闸/分闸状态 | 合 / 分 | 断路器、刀闸 |
| ALARM_STATUS | 报警状态 | 报警 / 正常 | 设备故障、温度过高 |
| CONTROL_MODE | 控制模式 | 本地 / 远程 | 操作权限模式 |
| FAULT_STATUS | 故障状态 | 故障 / 正常 | 电源通信异常、故障标志 |
| NONE | 无单位 | - | 占位符 |
