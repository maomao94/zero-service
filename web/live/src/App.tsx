...
import { Archive, ArrowRight, Camera, Check, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, Clipboard, Copy, Database, DoorOpen, History, LogOut, Maximize2, MessageSquare, Mic, Minimize2, MonitorUp, MoreHorizontal, Plus, RefreshCw, Search, Send, Settings2, Shield, ShieldCheck, Sparkles, UserRound, Users, Video, X, ZoomIn, ZoomOut } from 'lucide-react'
...
function RoomContent({ perms, onLeave, notify }: { perms: JoinPerms; onLeave: () => void; notify: (message: string, tone?: Toast['tone']) => void }) { const tracks = useTracks([Track.Source.Camera, Track.Source.ScreenShare], { onlySubscribed: false }); const [selectedTrack, setSelectedTrack] = useState<TrackReferenceOrPlaceholder | null>(null); const [showThumbnails, setShowThumbnails] = useState(true); const [thumbnailSize, setThumbnailSize] = useState(1); const [isFullscreen, setIsFullscreen] = useState(false); const [fit, setFit] = useState<'contain' | 'cover' | 'fill'>('cover'); const [zoom, setZoom] = useState(1); const mainVideoRef = useRef<HTMLDivElement>(null); useEffect(() => { if (!selectedTrack && tracks.length > 0) { const screenShare = tracks.find(t => t.source === Track.Source.ScreenShare); setSelectedTrack(screenShare || tracks[0]) } }, [tracks, selectedTrack]); useEffect(() => { if (selectedTrack) { const stillExists = tracks.some(t => t.participant.identity === selectedTrack.participant.identity && t.source === selectedTrack.source); if (!stillExists) setSelectedTrack(tracks[0] || null) } }, [tracks, selectedTrack]); const toggleFullscreen = async () => { if (!mainVideoRef.current) return; if (!document.fullscreenElement) { try { await mainVideoRef.current.requestFullscreen(); setIsFullscreen(true) } catch { notify('无法进入全屏模式', 'error') } } else { try { await document.exitFullscreen(); setIsFullscreen(false) } catch { notify('无法退出全屏模式', 'error') } } }; useEffect(() => { const handler = () => setIsFullscreen(!!document.fullscreenElement); document.addEventListener('fullscreenchange', handler); return () => document.removeEventListener('fullscreenchange', handler) }, []); const zoomBy = (delta: number) => setZoom((z) => Math.min(3, Math.max(1, Math.round((z + delta) * 100) / 100))); const cycleFit = () => setFit((f) => f === 'cover' ? 'contain' : f === 'contain' ? 'fill' : 'cover'); const fitLabel = fit === 'cover' ? '铺满' : fit === 'contain' ? '适应' : '拉伸'; const selectTrack = (track: TrackReferenceOrPlaceholder) => { setSelectedTrack(track); setZoom(1); setFit('cover') }; return <div className="room-content-layout">{showThumbnails ? <div className={`thumbnail-strip thumbnail-size-${thumbnailSize}`}><div className="thumbnail-header"><div><span className="thumbnail-title">参会人</span><small>{tracks.length} 路视频</small></div><div className="thumbnail-actions"><button className="icon-button small" onClick={() => setThumbnailSize((size) => Math.max(0, size - 1))} title="缩小参会人画面" aria-label="缩小参会人画面" disabled={thumbnailSize === 0}><ZoomOut size={14} /></button><button className="icon-button small" onClick={() => setThumbnailSize((size) => Math.min(2, size + 1))} title="放大参会人画面" aria-label="放大参会人画面" disabled={thumbnailSize === 2}><ZoomIn size={14} /></button><button className="icon-button small" onClick={() => setShowThumbnails(false)} title="收起参会人" aria-label="收起参会人"><ChevronUp size={15} /></button></div></div><div className="thumbnail-row">{tracks.map((track) => <button key={`${track.participant.identity}-${track.source}`} className={`thumbnail-item ${selectedTrack?.participant.identity === track.participant.identity && selectedTrack?.source === track.source ? 'active' : ''}`} onClick={() => selectTrack(track)} title={`查看 ${track.participant.name || track.participant.identity}`}><TrackTile track={track} /><span className="thumbnail-name">{track.participant.name || track.participant.identity}{track.participant.isLocal ? ' · 我' : ''}</span></button>)}</div></div> : <button className="participants-restore" onClick={() => setShowThumbnails(true)}><Users size={15} /><span>参会人</span><b>{tracks.length}</b><ChevronDown size={15} /></button>}<div className="main-video-area" ref={mainVideoRef}>{selectedTrack ? <TrackTile key={`${selectedTrack.participant.identity}-${selectedTrack.source}`} track={selectedTrack} fit={fit} zoom={zoom} /> : <div className="room-empty"><span className="empty-camera"><Video size={27} /></span><b>暂时没有可用视频</b><small>成员开启摄像头后会显示在这里</small></div>}<div className="video-toolbar"><button className="toolbar-button" onClick={() => zoomBy(-0.25)} title="缩小" disabled={zoom <= 1}><ZoomOut size={15} /></button><span className="toolbar-zoom">{Math.round(zoom * 100)}%</span><button className="toolbar-button" onClick={() => zoomBy(0.25)} title="放大" disabled={zoom >= 3}><ZoomIn size={15} /></button><button className="toolbar-button text" onClick={() => setZoom(1)} title="重置缩放">重置</button><button className="toolbar-button text" onClick={cycleFit} title="切换显示模式">{fitLabel}</button><button className="toolbar-button" onClick={toggleFullscreen} title={isFullscreen ? '退出全屏' : '全屏'}>{isFullscreen ? <Minimize2 size={15} /> : <Maximize2 size={15} />}</button></div></div><RoomControls perms={perms} onLeave={onLeave} notify={notify} /></div> }
function TrackTile({ track, fit = 'cover', zoom = 1 }: { track: TrackReferenceOrPlaceholder; fit?: 'contain' | 'cover' | 'fill'; zoom?: number }) { const title = track.participant.name || track.participant.identity; const isSubscribed = track.publication?.isSubscribed && track.publication.track; return <div className="track-tile" data-source={track.source === Track.Source.ScreenShare ? 'screenShare' : 'camera'}>{isSubscribed ? <VideoTrack trackRef={track as TrackReference} className="track-video" autoPlay muted={track.participant.isLocal} playsInline style={{ objectFit: fit, transform: zoom !== 1 ? `scale(${zoom})` : undefined }} /> : <div className="track-offline"><span className="avatar tiny">{initials(title)}</span><span>{title}</span><small>视频未开启</small></div>}<div className="track-label"><span className="avatar tiny">{initials(title)}</span><span>{title}{track.participant.isLocal ? ' · 我' : ''}</span></div></div> }
