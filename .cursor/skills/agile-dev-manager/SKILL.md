...
`CP-开发流程/template/` 与 `template/README.md` 是仓库内标准文件包；本技能本节为 Agent 可执行的同构说明（人类速查可继续看 `开发规范.md`）。
...
### 10.4 文档化流程（与 template README 一致）
...
| 类型 | 位置 |
| --- | --- |
| 工作流速查 | `CP-开发流程/开发规范.md` |
| 敏捷与文档体系（完整） | 本技能（`agile-dev-manager`） |
| 角色职责 | `ai-team` 技能 |
| 项目会话/编码规则 | `.trae/rules/rule.md`、`AGENTS.md` |
| 注入式技术规范 | `.trellis/spec/`（Trellis `/start` 等） |

---

## 十一、示例模板

以下为 5 个核心文件的正文结构参考，与 `CP-开发流程/template/*.md` 一一对应；`template/README.md` 说明如何复制、占位符与跨文档关系（摘要见**第十节**）。
...
本节与 `CP-开发流程/开发规范.md` **同构**：该文件给人类一页纸速查；执行 Sprint 时以本技能全文 + 本节为 Agent 的「规范 + 工具」锚点，避免与仓库文档重复维护时失配。
...
| 阶段 | 动作 |
| --- | --- |
| 编码前 | 读 `.trellis/spec/` → 检索项目代码 → 确认技术栈与边界 |
| 编码中 | 遵守注入规范 → 需要框架细节时查 `zero-skills` 技能 → 复用已有组件与模式 |
| 编码后 | `go build` → 工具类/关键逻辑单测 → proto/api 等注释与契约自检 |

Trellis `/check`、`.agent` 工作流中的检查清单用于落实「编码后」与 Review 阶段。

### 12.3 工具链与职责

| 能力 | 使用方式 |
| --- | --- |
| Trellis 命令 | 见第三节「Trellis 命令集成对照表」与 `开发规范` 中 Sprint 四阶段列 |
| 项目规范注入 | `.trellis/spec/`；会话启动时 `/start` 或等效加载 |
| 任务与会话 | `.trellis/tasks/`、`.trellis/workspace/`、`.trellis/.current-task`（见会话启动步骤） |
| go-zero 与微服务模式 | `zero-skills` 技能 |
| 本机命令、部署含 Dockerfile 的模块 | `dev-environment`、`module-deploy` 技能（按需） |

### 12.4 当模板或规范在仓库中变更时

若修改了 `CP-开发流程/template/`、`开发规范.md` 或本技能，按仓库 `rule` 中约定同步到 `.trellis/spec/` 等下游（以 `.trae/rules/rule.md` 中「同步链」为准），避免文实不一致。
