-- KingbaseES 业务账号体系脚本（在需要授权的业务库上执行，幂等可重复）
--
-- 用法（任选其一，连接到业务库）:
--   1) 数据库管理工具连接业务库后执行本文件
--   2) docker exec -i kingbase ksql -Usystem -d <业务库名> -p 54321 < deploy/kingbase/roles.sql
--      （容器内 local 连接为 trust 免密）
...
do $$ begin
  if not exists (select 1 from pg_roles where rolname = 'app_user') then
    create role app_user with login password 'app123456';
  end if;
  if not exists (select 1 from pg_roles where rolname = 'query_user') then
    create role query_user with login password 'query123456';
  end if;
  if not exists (select 1 from pg_roles where rolname = 'admin_user') then
    create role admin_user with login password 'admin123456' createrole createdb;
  end if;
end $$;
...
--    注意: 本环境修改用户密码使用 alter user；alter role 对已有用户可能报“不是一个 role”。
