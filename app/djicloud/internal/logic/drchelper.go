...
func toDrcMqttBroker(cfg mqttx.MqttConfig, drcAddress string) djisdk.DrcMqttBroker {
	...
	if drcAddress != "" {
		if u, err := url.Parse(drcAddress); err == nil && u.Host != "" {
			broker.Address = u.Host
			broker.EnableTLS = u.Scheme == "tcps" || u.Scheme == "mqtts"
		} else {
			broker.Address = drcAddress
		}
	} else if len(cfg.Broker) > 0 {
		addr := cfg.Broker[0]
