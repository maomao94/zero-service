...
type DjiFlyRegion struct {
	...
	Name         string `gorm:"column:name;type:varchar(128);not null;default:'';comment:用户自定义飞行区名称（来自 gRPC 请求，如航点A区、禁飞区-1号）"`
	FileId       string `gorm:"column:file_id;type:varchar(64);uniqueIndex;not null;comment:文件唯一标识(UUID)，用于业务关联"`
	BucketName   string `gorm:"column:bucket_name;uniqueIndex:uq_dji_fly_region_bucket_name_file_name;not null;default:'';comment:OSS 存储桶名称"`
	FileName     string `gorm:"column:file_name;uniqueIndex:uq_dji_fly_region_bucket_name_file_name;not null;comment:OSS 文件对象 key（自动生成，如 dfence_abc123.json）"`
...
type DjiFlyRegionSyncStatus struct {
	...
	FlyRegionID string `gorm:"column:fly_region_id;type:varchar(36);index;comment:关联 dji_fly_region.id"`
