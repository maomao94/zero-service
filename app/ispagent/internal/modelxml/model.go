// Package modelxml defines ISP protocol model types and provides streaming XML generation
// for device point model files (Device_Model) and patrol device model files (PatrolDevice_Model).
package modelxml
...
type DevicePointModel struct {
	...
	BayID string `xml:"bay_id,attr"`
	// BayName 间隔名称
	BayName string `xml:"bay_name,attr"`
	// MainDeviceID 主设备 ID，点位所属主设备，和中台设备编码保持一致
	MainDeviceID string `xml:"main_device_id,attr"`
	// MainDeviceName 主设备名称
	MainDeviceName string `xml:"main_device_name,attr"`
	...
	// DeviceType 主设备类型（1=油浸式变压器, 2=断路器, 3=组合电器, 4=隔离开关, 5=开关柜, 6=电流互感器, 7=电压互感器,
	// 8=避雷器, 9=并联电容器组, 10=干式电抗器, 11=串联补偿装置, 12=母线及绝缘子, 13=穿墙套管, 14=消弧线圈,
	// 15=高频阻波器, 16=耦合电容器, 17=高压熔断器, 18=中性点隔直装置, 19=接地装置, 20=端子箱及检修电源箱,
	// 21=站用变压器, 22=站用交流电源系统, 23=站用直流电源系统, 24=设备构架, 25=辅助设施, 26=土建设施,
	// 27=独立避雷针, 28=电力电缆, 29=二次屏柜, 30=消防系统）
	DeviceType string `xml:"device_type,attr"`
	// MeterType 表计类型（1=油位表, 2=避雷器动作次数表, 3=泄漏电流表, 4=SF6压力表, 5=液压表,
	// 6=开关动作次数表, 7=油温表, 8=档位表, 9=气压表）
	MeterType string `xml:"meter_type,attr"`
	// AppearanceType 辅助设施类型（1=电子围栏, 2=红外对射, 3=泡沫喷淋, 4=消防水泵, 5=消防栓,
	// 6=消防室, 7=设备室, 8=照明灯, 9=摄像头, 10=水位线, 11=排水泵, 12=沉降监测点）
	AppearanceType string `xml:"appearance_type,attr"`
	// SaveTypeList 采集/保存文件类型列表，多个类型用逗号分隔
	SaveTypeList string `xml:"save_type_list,attr"`
	// RecognitionTypeList 识别类型列表，多个类型用逗号分隔
	RecognitionTypeList string `xml:"recognition_type_list,attr"`
	// Phase 相位（1=A相, 2=B相, 3=C相），多个相位用逗号分隔
	Phase string `xml:"phase,attr"`
	// DeviceInfo 备注信息，用于描述设备点位的文字信息；可能包含 JSON 扩展数据
	DeviceInfo string `xml:"device_info,attr"`
	// DataType 设备点位支持的数据来源，按位定义（第0位=摄像机, 第1位=机器人, 第2位=无人机, 第3位=声纹, 第4位=在线监测）
	DataType string `xml:"data_type,attr"`
	// LowerValue 正常范围下限，选填
	LowerValue string `xml:"lower_value,attr"`
	// UpperValue 正常范围上限，选填
	UpperValue string `xml:"upper_value,attr"`
	// VideoPos 关联视频编码及预置位，JSON 格式；可能包含 device_code/device_pos/robot_code/robot_pos/uav_code/uav_pos
	VideoPos string `xml:"video_pos,attr"`
	// PointType 重要等级（1=I类, 2=II类, 3=III类）
	PointType string `xml:"point_type,attr"`
	// LabelAttri 标签属性，点位标签属性，多个附加属性逗号分隔（1=人工关注）
	LabelAttri string `xml:"label_attri,attr"`
}
