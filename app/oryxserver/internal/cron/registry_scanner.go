...
func (s *RegistryScanner) processStale(ctx context.Context, store *relay.Store, registry *relay.RelayRegistry, entry *relay.StaleEntry) {
	...
	lock, ok, err := store.Lock(ctx, uid)
	if err != nil {
		logx.Errorf("[registry-scanner] lock failed: uid=%s err=%v", uid, err)
		return
	}
	if !ok {
		return // 别人在操作，跳过
	...
	latest, err := store.GetState(ctx, uid)
	if err != nil {
		logx.Errorf("[registry-scanner] get state failed: uid=%s err=%v", uid, err)
		return
	}
	if latest == nil {
		// state 已删，清理 registry 索引
		_ = store.RemoveFromRegistry(ctx, uid)
		return
	}

	// 检查是否有活跃租约
	has, err := store.HasLease(ctx, uid)
	if err != nil {
		logx.Errorf("[registry-scanner] check lease failed: uid=%s err=%v", uid, err)
		return
	}
	if has {
		// 有 lease 说明正在运行，刷新 registry score
		_ = store.RenewRegistry(ctx, uid)
		return
	}
	...
	if latest.PendingReconcile {
		...
		if scoreAge > relay.PendingStaleThreshold {
			logx.Infof("[registry-scanner] pending stuck uid=%s score_age=%v, resetting and re-enqueuing", uid, scoreAge)
	...
	if err := registry.EnqueueReconcile(ctx, uid, latest.Source, latest.RetryCount); err != nil {
		logx.Errorf("[registry-scanner] enqueue reconcile failed: uid=%s err=%v", uid, err)
		return
	}
	latest.PendingReconcile = true
	if err := store.SaveState(ctx, latest); err != nil {
		logx.Errorf("[registry-scanner] save state failed: uid=%s err=%v", uid, err)
	}
	logx.Infof("[registry-scanner] enqueue reconcile: uid=%s retryCount=%d", uid, latest.RetryCount)
}
