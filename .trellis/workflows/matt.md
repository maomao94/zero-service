# Development Workflow — matt variant

> **Matt 集成变体**：在标准 Trellis 生命周期之上，将 Matt Pocock 工程方法技能（`grilling`、`research`、`tdd`、`diagnosing-bugs`、`code-review`、`prototype`、`retro`）织入对应阶段。Trellis 管生命周期与工件落点，Matt 管方法。按任务选用：`task.py create --workflow matt` 或对 active task 执行 `task.py workflow matt`。
...
Matt method layer: stress-test requirements via `grilling` / `grill-me`; primary-source research via `research` (persist to `{TASK_DIR}/research/`); design spikes via `prototype`.
...
Matt method layer: test-first via `tdd`; bug/perf diagnosis via `diagnosing-bugs` before any fix; two-axis review via `code-review` before the final 2.2 pass. Matt skills never commit — Phase 3.4 protocol only.
...
### Engineering-Method Routing (Matt Pocock skills)

Matt skills are the method layer inside this workflow's phases: Trellis owns the lifecycle gate (consent -> planning -> start -> commit) and artifact destinations; the Matt skill owns how the work is done.

- Requirement stress-testing during Phase 1.1 -> `grilling` / `grill-me` (requirements -> `prd.md`); blocked on someone else's knowledge -> `to-questionnaire`.
- Primary-source research during Phase 1.2 -> `research` (findings MUST persist to `{TASK_DIR}/research/`).
- Multi-deliverable breakdown -> vertical slices a la `to-tickets`, registered as Trellis child tasks (`task.py create --parent`); very large fuzzy scopes -> `wayfinder`-style decision map, one decision per session.
- Test-first discipline during Phase 2.1 -> `tdd` (red-green loop at pre-agreed seams).
- Bug / performance diagnosis during Phase 2 -> `diagnosing-bugs` (6-phase loop) before any fix.
- Two-axis review before the final Phase 2.2 pass -> `code-review` (Standards axis + Spec axis); `trellis-check` remains the gate.
- Phase 3.2 retrospective -> `trellis-break-loop` first; `retro` for agent-environment improvement candidates.
- Phase 3.3 knowledge capture -> `trellis-update-spec` for `.trellis/spec/`. No glossary file and no ADRs in this repo.
- Phase 3.4 commit -> Trellis batched-commit protocol only; Matt skills never commit or push.
- Unsure which Matt skill fits the current intent -> `ask-matt` (router over the whole Matt skill set, idea -> ship).

Conflict rule: when a Matt skill's instruction collides with a Trellis phase gate, Trellis wins. Outside an active task, Matt skills run standalone per their own conventions (see `AGENTS.md` "Workflow 分工").
...
**Matt method layer**: use `grilling` / `grill-me` to stress-test the plan (outcomes still land in `prd.md`); when blocking information lives in someone else's head, turn it into a `to-questionnaire`; for fuzzy scopes too large for one session, build a `wayfinder`-style decision map and resolve one decision per session.
...
**Matt method layer**: the `research` skill fits this step — investigate against high-trust primary sources (official docs, source code, specs) and write findings as a cited Markdown file. Findings MUST still land in `{TASK_DIR}/research/`; the standalone default (`docs/research/`) applies only outside a task.
...
**Matt method layer**: implement test-first via `tdd` (red-green loop at pre-agreed seams; refactor outside the loop); when the task is a bug fix or performance regression, run `diagnosing-bugs` (6-phase loop) before writing any fix; use `prototype` for throwaway design validation — never as production code.
...
**Matt method layer**: before or alongside the final pass, run `code-review` along its two axes — Standards (repo `.trellis/spec/` compliance) and Spec (task-artifact fidelity). Its findings feed fixes; `trellis-check` remains the quality gate that auto-fixes and verifies lint/typecheck.
...
**Matt method layer**: complement with `retro` for agent-environment improvements (navigation, auto-checks, coding standards, AGENTS.md, tool economy, no-ops, information access) — session-level learning beyond the single bug.
...
**Note**: this repo keeps no glossary file and no ADRs — cross-task conventions go to `.trellis/spec/`, in-task design decisions to the task's `design.md`.
