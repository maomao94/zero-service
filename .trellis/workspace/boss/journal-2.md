...
## Session 94: 项目文档全面优化与 Trellis spec 更新

**Date**: 2026-06-29
**Task**: 项目文档全面优化与 Trellis spec 更新
**Branch**: `master`

### Summary

优化全部项目文档结构：README 从142行精简至91行，移除实验性功能(alarm/streamevent服务)；DJI云平台文档覆盖88个RPC接口；错误码文档改用extproto六位编码体系；socketio.md从1060行压缩至263行；iec104/trigger/kml-kmz等文档精简重组；更新trellis spec文档组织指南
...
| Hash | Message |
|------|---------|
| `7b071f14` | (see git log) |
| `0ad71a25` | (see git log) |
...
## Session 95: Spec tidy + online_cache fix

**Date**: 2026-06-29
**Task**: Spec tidy + online_cache fix
**Branch**: `master`

### Summary

清理: 删除实验性 spec (uix-framework, bubble-tea-tui-guide), 更新索引; 修复: netx-guidelines classifyNetErr 引用错误, 移除全部 spec 中的行号引用 (21处), 修正 online_cache 参数名/注释; 验证: 无占位文本残留, 索引与实际文件一致
...
| Hash | Message |
|------|---------|
| `8501b986` | (see git log) |
| `7f93301b` | (see git log) |
...
按 DJI 协议重塑 flight_areas SDK: method.go 新增 flight_areas_drone_location/flight_areas_sync_progress; protocol.go 重构数据模型(移除 UnlockLicenseFile 复用); handler.go 添加事件分发; 新增 ErrSkipRequestReply sentinel 支持旧 method 不回复; app/djicloud 添加对应 hooks
...
| Hash | Message |
|------|---------|
| `fe73aeb2` | (see git log) |
| `cda8f268` | (see git log) |
| `dbd7a151` | (see git log) |
...
## Session 97: djisdk 自定义飞行区 GeoJSON 处理 + djcloud OSS 配置与平台接口

**Date**: 2026-07-01
**Task**: djisdk 自定义飞行区 GeoJSON 处理 + djcloud OSS 配置与平台接口
**Branch**: `master`

### Summary

完成自定义飞行区完整功能：geofence.go GeoJSON 工具、Submit/Delete/List RPC、DjiFlyRegion+DjiFlyRegionSyncStatus 模型、FlightAreasGet 实时签名+并发、FlightAreasSyncProgress 落库、OSS 配置、proto 注释规范化
...
## Session 98: DJI 飞行区优化 + Trellis Spec Bootstrap

**Date**: 2026-07-01
**Task**: DJI 飞行区优化 + Trellis Spec Bootstrap
**Branch**: `master`

### Summary

1. DJI 飞行区 GeoJSON 优化：DjiFlyRegion 模型增加 FileId 唯一索引；文件名改为 {geofence_type}_{fileId}.json；feature id 各独立 UUID；SubmitCustomFlyRegion 返回 file_id；新增 DeleteCustomFlyRegionByFileId（按 file_id 删除，device_sn 从 DB 反查）；DeleteCustomFlyRegion 改为清空所有并返回 deleted_file_ids；ListFlyRegions 返回 file_id 2. Trellis Spec Bootstrap：重写 directory-structure.md（完整 19 app + 5 aiapp + 2 socketapp + 38 common 包清单）；新建 geofence-guidelines.md（飞行区全流程规范）
...
| Hash | Message |
|------|---------|
| `9d71ecb2` | (see git log) |
| `5516eec4` | (see git log) |
| `f7ddca00` | (see git log) |
| `d3f410ed` | (see git log) |
| `5be85525` | (see git log) |
...
## Session 99: ListFlyRegions 增加 sign_url 签名地址

**Date**: 2026-07-01
**Task**: ListFlyRegions 增加 sign_url 签名地址
**Branch**: `master`

### Summary

ListFlyRegionsReq 新增 sign_url 参数，FlyRegionInfo 新增 url 字段；sign_url=true 时使用 mr.Finish 并发生成 OSS 7 天签名下载地址
...
## Session 100: Trellis Spec 修复 — geofence/djicloud-models 同步代码现状

**Date**: 2026-07-01
**Task**: Trellis Spec 修复 — geofence/djicloud-models 同步代码现状
**Branch**: `master`

### Summary

修复 geofence-guidelines.md: 纠正 fileId/featureId 关系（fileId 仅文件名，feature id 各独立 UUID）、补全 SubmitCustomFlyRegionRes 响应类型、新增 ListFlyRegions+sign_url 章节。修复 djicloud-models.md: DjiFlyRegion 补全 FileId/BucketName 字段和唯一键、FileName 格式更新为 {geofence_type}_{fileId}.json
...
## Session 102: gnetx: shared ReplyPool, composite TID, Dialer, enhanced LengthPrefix codec, DebugSerializer

**Date**: 2026-07-03
**Task**: gnetx: shared ReplyPool, composite TID, Dialer, enhanced LengthPrefix codec, DebugSerializer
**Branch**: `master`

### Summary

重构 gnetx 包：Server/Client 共享 ReplyPool（消除每连接建池开销）、复合 TID（sessionID|tid）、新增短连接 Dialer（共享 gnet.Client 引擎+懒加载+Promise 匹配）、LengthPrefixCodec 新增 WithLeadingBytes/WithTrailingBytes/WithStripBytes、新增 DebugSerializer（base64 调试日志）、修复 5 个竞态/泄漏问题（ErrIncompletePacket 误关连接、Close 同步、replyPool 泄漏、重连竞态、applyFrameLimit 缺失）、更新所有 Trellis spec。
...
## Session 103: Spec Bootstrap: einox/mcpx 规范 + gnetx 心跳更新

**Date**: 2026-07-03
**Task**: Spec Bootstrap: einox/mcpx 规范 + gnetx 心跳更新
**Branch**: `master`

### Summary

新增 einox-guidelines.md（Agent 工厂/ChatModel/工具管理/中间件/Runtime/存储）和 mcpx-guidelines.md（Client 多连接/Server 鉴权/CallToolWrapper/异步任务）；更新 gnetx/client.md 补充心跳配置章节；更新 backend/index.md 加入新规范索引。
...
## Session 104: flowx Workflow 封装开发

**Date**: 2026-07-03
**Task**: flowx Workflow 封装开发
**Branch**: `master`

### Summary

common/flowx 包：基于 Azure/go-workflow 的 go-zero 集成封装。提供 New() 构造函数、8个 FlowOption、LoggingStepInterceptor/StepFields/AttemptFields 三个拦截器，17个单测全部通过。
...
| Hash | Message |
|------|---------|
| `2751e54e` | (see git log) |
| `52133a2b` | (see git log) |
