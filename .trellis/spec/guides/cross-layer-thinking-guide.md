...
- **分布式锁持有期间发送广播**——广播接收方需要拿同一把锁执行本地操作，发送方持有锁会导致接收方跳过。正确顺序：拿锁 → 本地操作 → **释放锁** → 广播。
- **`lock.ReleaseCtx(ctx)` 用请求 ctx**——请求超时后释放失败，锁只能等 TTL。所有锁释放必须用 `lock.Release()`（内部 `context.Background()`）。
- **Asynq TaskID 策略与重试语义不匹配**——幂等任务用固定 ID（去重），需要重试的任务用自动生成 ID（允许重复入队）。混用会导致重试被拒。
- **直接使用 `uuid.New()` 而非 `IdUtil`**——项目约定所有业务 ID 必须通过 `IdUtil` 生成（`SimpleUUID()` 生成无杠 UUID v7，`NextId()` 生成 Redis 序号）。直接使用 uuid 包会导致格式不一致、无法追溯。
