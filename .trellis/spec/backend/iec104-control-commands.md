...
| SetpointFloat | `string` | `convertor.ToFloat(in.Value)` → `float32` | `float32` | `SetpointCommandFloatInfo.Value: float32` |
...
**Producer**（logic 层，集群模式）：

所有控制命令统一使用 `PushPbBroadcastWithAck`，等待 ACK 回复：
...
> **Common Mistake**: `cli != nil` 分支发送成功后必须 `return &res, nil`，否则 fall through 到最后的 `return nil, error("IEC客户端不存在")`，导致成功也被当失败。

**Consumer**（mqtt/broadcast.go）：

所有命令统一使用 `publishAckReply`，不再 `return err`：

```go
case ieccaller.IecCaller_SendXxxCommand_FullMethodName:
    // ... unmarshal, GetClient ...
    if err = cli.SendXxxCmd(...); err != nil {
...
| 命令类型 | ASDU 回执 | ACK 时机 | 响应体 |
|---------|----------|---------|--------|
| 带类型命令 (SendSingleCommand 等) | 有 | 等待 ASDU ACK 后回复 | `CommandAck.Value` JSON |
| 其他命令 (SendInterrogationCmd 等) | 无 | 发送成功后立即回复 | `{}` |

**publishAckReply 日志**：

- 入口（Info）：`publishAckReply tId=xxx method=... success=true/false errorKind=...`
- MQTT 发送失败（Error）：`failed to push ack to mqtt: tId=xxx method=...`
- MQTT 发送成功（Debug）：`ack reply sent: tId=xxx method=...`
- tId 为空（Debug）：`publishAckReply skipped: empty tId`
...
| SendSetpointFloat value 非数字格式 | `validator.IsNumberStr` 校验失败 → `Code__1_06_RPC` |
| SendSetpointFloat value 有效数字超 7 位 | `tool.CountSignificantDigits` 校验失败 → `Code__1_06_RPC` |
...
#### 4.2 SendSetpointFloat 精度契约

**Design Decision**: `SendSetpointFloatReq.value` 和 `SendSetpointFloatRes.value` 使用 `string` 类型而非 `float`。

**Context**: IEC 104 `C_SE_NC_1` 短浮点设点使用 IEEE 754 单精度（float32）。proto 使用 `float` 类型时，protobuf JSON 序列化会将 float32 提升为 float64 输出完整精度（如 `51.880001068115234`），导致 API 响应展示混乱。使用 `string` 类型可以精确控制输出格式。

**校验链路**（fail-fast，在获取客户端之前执行）：
1. `validator.IsNumberStr(in.Value)` — lancet 通用正则校验，拒绝非数字格式
2. `tool.CountSignificantDigits(in.Value)` — 有效数字不超过 7 位（float32 精度上限）
3. `convertor.ToFloat(in.Value)` — lancet 解析为 float64，再转 float32 发送

**响应格式**：`convertor.ToString(ackValue)` — 从站 ACK 的 float32 原始值转字符串返回，小数位由上游设备决定，不做截断。

**Wrong**:
```go
// 不要在协议层截断小数位
value = float32(mathutil.RoundToFloat(float64(ackValue), 2))
// 不要在获取客户端之后才校验
cli, _ := l.svcCtx.ClientManager.GetClientOrNil(...)
fv, _ := convertor.ToFloat(in.Value)  // 校验太晚
```

**Correct**:
```go
// 先校验再获取客户端
if !validator.IsNumberStr(in.Value) { return error }
sigDigits := tool.CountSignificantDigits(in.Value)
if sigDigits > 7 { return error }
fv, _ := convertor.ToFloat(in.Value)
// ... 获取客户端、发送命令 ...
// ACK 原始值直接转字符串
return &ieccaller.SendSetpointFloatRes{Value: convertor.ToString(ackValue)}
```

**测试**: `common/tool/util_test.go` 的 `TestCountSignificantDigits` 覆盖边界用例。
...
## Convention: Proto 字段顺序与 DB Model 一致

**What**: proto message 的字段顺序必须与数据库 model struct 的字段顺序一致。

**Why**: 
- 保持 proto 定义与 DB schema 的视觉一致性，便于维护
- copier 自动映射依赖字段名匹配，顺序不影响功能，但顺序一致减少认知负担
- `PbDevicePointMapping` 示例：`description` 在 `ext1` 前面，与 model struct 一致

**Example**:
```protobuf
// Correct - description 在 ext1 前面，与 model 顺序一致
message PbDevicePointMapping {
  int64 id = 1;
  string createTime = 2;
  // ...
  int32 enablePush = 10;
  string description = 11; // 在 ext1 前面
  string ext1 = 12;
  string ext2 = 13;
  // ...
}

// Wrong - description 放在最后，与 model 顺序不一致
message PbDevicePointMapping {
  // ...
  string ext1 = 11;
  string ext2 = 12;
  // ...
  string description = 16; // 应该在 ext1 前面
}
```

**Related**: `model/devicepointmappingmodel_gen.go` 字段顺序是权威来源。
