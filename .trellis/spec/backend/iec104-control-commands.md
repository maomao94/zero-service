...
> ieccaller 新增控制方向 gRPC 接口的 canonical source。覆盖 proto 定义、client 封装、logic 编排、Kafka 广播全链路。

## When to read

- 新增或修改 ieccaller 控制方向（C_*）gRPC 接口。
- 修改 `common/iec104/client/core.go` 的 Send*Cmd 方法或 doSend 分发逻辑。
- 修改 `kafka/broadcast.go` 消费者 case。
- 对接方询问控制命令的 typeId 和字段含义。

## 全链路文件清单

修改顺序强制：proto → gen.sh → client → logic → server → kafka consumer。
...
| 6 | `app/ieccaller/kafka/broadcast.go` | 新增 consumer case |
...
- Scope: 覆盖 6 层文件修改，从 proto 到 Kafka consumer。
...
**Consumer**（kafka/broadcast.go，必须手动添加）：
...
case ieccaller.IecCaller_SendSingleCommand_FullMethodName:
    ...
    if err != nil {
        l.publishAckReply(ctx, key, broadcastBody.BroadcastGroupId, ..., false, "", err)
        return nil
    }
    value, ok := ack.Value.(bool)
    if !ok {
        l.publishAckReply(ctx, key, ..., false, "", fmt.Errorf("unexpected ack value type"))
        return nil
    }
    resJson, _ := jsonx.Marshal(&ieccaller.SendSingleCommandRes{Value: value})
    l.publishAckReply(ctx, key, ..., true, string(resJson), nil)
```

**traceId 约定**：使用 Kafka message key 传递，`PushWithKey(ctx, traceId, body)`。Consumer `Consume(ctx, key, value)` 中 `key` 即 traceId。`BroadcastBody` 和 `BroadcastAckBody` 中不再存储 traceId 字段。
...
kafka:  case IecCaller_SendDoubleCommand_FullMethodName: ... cli.SendDoubleCmd(ctx, ...)
```

**Base** — 现有 `SendCommand` 通用接口（保留用于选控、自定 typeId 等高级场景）。

**Bad** — 只加 proto + logic，不加 kafka/broadcast.go consumer case → cluster 部署时其他实例收不到广播命令。
...
- [ ] kafka/broadcast.go 每个新 RPC 有对应 case
- 集成测试（需真实 IEC 104 从站或模拟器）：gRPC 调用 → 从站返回激活确认 COT=7

---

## Scenario: 集群 Broadcast ACK Reply

### 1. Scope / Trigger

- Trigger: 集群部署下，ACK 型控制命令（7 个 typed 命令）的 broadcast 分支改用 `PushPbBroadcastWithAck` 替代原有的 fire-and-forget `PushPbBroadcast`。
- Scope: ServiceContext 新增 `PushPbBroadcastWithAck`、`BroadcastReplyPool`、`KafkaBroadcastAckPusher`；kafka/broadcast.go 消费者改用 `client.WithAck()` 执行并发布 ACK reply；kafka/broadcast_ack.go 消费 ACK reply 并 resolve replypool。
...
**PushPbBroadcastWithAck 方法**：
```go
func (svc ServiceContext) PushPbBroadcastWithAck(ctx context.Context, method string, in any, res any) error
```
- 生成 traceId (UUID) → 在 BroadcastReplyPool 注册 → PushWithKey(traceId) 到 BroadcastTopic → Await → 检查 ErrorKind → 解析 ResponseBody 到 res
...
**Broadcast 消费者 publishAckReply helper**：
```go
...
```
- 用 `PushWithKey(ctx, ackKey, data)` 发布到 BroadcastAckTopic
- 自动从 error 中提取 ErrorKind（timeout/duplicate/unknown）
...
**traceId 传递契约**：使用 Kafka message key 传递，不在 body 中冗余存储。
- `PushPbBroadcastWithAck` → `PushWithKey(ctx, traceId, body)` → consumer `Consume(ctx, key, value)` 中 `key` = traceId
- `publishAckReply` → `PushWithKey(ctx, ackKey, data)` → ACK consumer `Consume(ctx, key, value)` 中 `key` = traceId
- `BroadcastAckBody` 不需要 `TraceId` 字段

**多实例竞争保护**：非 owner 实例在 `GetClient` 失败时静默跳过（return nil），不发送 ACK reply。只有持有目标 client 的实例执行 `client.WithAck()` 并发布 response。避免非 owner 实例抢先发送 failure ACK 导致 winner 实例的 success ACK 被忽略。

**ErrorKind 映射**：
- `publishAckReply` 对传入的 error 做 `errors.Is(err, antsx.ErrReplyExpired)` 判断 → ErrorKind="timeout"
- `publishAckReply` 对传入的 error 做 `errors.Is(err, antsx.ErrDuplicateID)` 判断 → ErrorKind="duplicate"
...
- 上游 `wrapCommandAckError` 自动映射到正确的 protobuf 错误码

**配置契约**：
- `KafkaConfig.BroadcastAckTopic` 默认值 `iec-broadcast-ack`
- ACK consumer group 为 `broadcastInstanceId + "-ack"`（启动时自动生成 UUID，带 `iec-caller-` 前缀）
- 无需手动配置，每个实例启动时自动生成唯一标识，保证 Kafka fanout 语义
...
| KafkaBroadcastPusher 为 nil | `fmt.Errorf("kafka broadcast pusher is nil")` |
| BroadcastReplyPool 为 nil | `fmt.Errorf("broadcast reply pool is nil")` |
| traceId 生成失败 | `fmt.Errorf("generate traceId error: %w", err)` |
| replypool 注册重复 traceId | `fmt.Errorf("register reply pool error: %w", err)` |
| Kafka 推送 ACK reply 失败 | 仅日志 Error，不阻塞 |
...
**Good** — 集群 ACK 完整流程：
```
1. logic: cli==nil → PushPbBroadcastWithAck → 生成 traceId → 注册 replypool
2. PushWithKey(traceId) → Kafka BroadcastTopic
3. owner 实例: Consume(key=traceId, value=body) → GetClient(成功)
4. owner 实例: cli.SendXxxCmd(WithAck()) → Await → ack.Value
5. owner 实例: publishAckReply(key=traceId, originGroupId, true, resJson, nil)
6. PushWithKey(traceId) → Kafka BroadcastAckTopic
7. origin 实例: Consume(key=traceId) → Resolve(traceId, ackBody)
8. PushPbBroadcastWithAck: ack.Success==true → unmarshal → return typed res
```

**Base** — owner 实例执行命令失败：
```
1-3. 同上
4. owner 实例: cli.SendXxxCmd(WithAck()) → err
5. owner 实例: publishAckReply(key=traceId, ..., false, "", err)
6-8. PushPbBroadcastWithAck: ErrorKind 映射 → 返回对应 sentinel error
```

**Bad** — GetClient 失败时发送 failure ACK（已修复）：
```go
// Wrong: 非 owner 实例抢先发 failure ACK，导致 origin 永远收不到 owner 的 success ACK
if err != nil {
    l.publishAckReply(ctx, key, ..., false, "", err)
    return nil
}

// Correct: 非 owner 实例静默跳过
if err != nil {
    logx.WithContext(ctx).Errorf("get client error: %v", err)
    return nil
}
...
**Bad** — 在 BroadcastAckBody 中冗余存储 traceId（已修复）：
```go
// Wrong: traceId 在 body 中存放，与 Kafka key 重复
type BroadcastAckBody struct {
    BroadcastGroupId string `json:"broadcastGroupId"`
    TraceId          string `json:"traceId"`    // 冗余
    ...
}

// Correct: traceId 由 Kafka message key 传递，Consumer 直接读 key
type BroadcastAckBody struct {
    BroadcastGroupId string `json:"broadcastGroupId"`
    // traceId 由 Kafka key 传递，不在 body 中存储
    ...
}
```
...
- [ ] `publishAckReply` 空 key 不发送 ACK reply
- [ ] `BroadcastReplyPool.Close` 在 `ServiceContext.Close` 中调用
- [ ] 非 ACK 型 broadcast（SendCommand、总召等）保持 fire-and-forget
...
**Wrong** — 遗漏 Kafka consumer case：
```go
// kafka/broadcast.go 的 switch 中没有新命令的 case
default:
