...
1. **预置分支**（`tryDispatchEventNotify`）：switch on method，命中已注册 handler 则执行。handler 返回 `error` 时通过 `ResultFromError(err)` 提取 `PlatformError.Code` 作为 events_reply 的 result
2. **未命中 method**：返回 `PlatformResultHandlerError`（非 OK），方便定位未注册事件
3. **默认**：未命中且无 handler 时打印 payload 日志
