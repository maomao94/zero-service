...
| `geofence.go` | 自定义飞行区 GeoJSON 工具：`NewGeofencePolygonFeature`/`NewGeofenceCircleFeature`/`NewGeofenceFeatureCollection`/`ToJSON` |
| `geofence_test.go` | GeoJSON 序列化输出正确性测试 |
...
16 个 handler 回调聚合在未导出 `handlers` struct（`option.go`），`Client` 和 `clientOptions` 均按值持有。`buildClient` 一行 `c.handlers = opt.handlers` 完成映射。
...
| `WithFlightAreasSyncProgressHandler` | `func(ctx, gatewaySn, *FlightAreasSyncProgressEvent) error` | up | events |
| `WithFlightAreasDroneLocationHandler` | `func(ctx, gatewaySn, *FlightAreasDroneLocationEvent) error` | up | events |
...
- `RequestHandler` `func(ctx, gatewaySn, *RequestMessage) (output any, err error)` — output 写入 _reply.data.output；返回 `ErrSkipRequestReply` 时跳过回复（见 `error.go`）
...
- `ErrSkipRequestReply` — `RequestHandler` 返回此错误时 `HandleRequests` 跳过 `requests_reply` 发布。用于新平台不处理某些旧 method 的场景（参考 `mqtt_request_up.go`）
