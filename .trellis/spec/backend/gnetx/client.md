...
收发：OnTraffic → decode → Response 匹配（resolveResponse + 未匹配丢弃，同 Server）→ dispatch
    ↓ 断连
OnClose → c.sess.CAS(nil) → cn.closeFromEventLoop() → startReconnect
    ↓ 固定间隔后
c.dial() → OnOpen → 收发恢复
...
Close() → Shutdown(ctx, ShutdownTimeout)
    ...
    → gcli.Stop()（停止事件源，保证不再 asyncWG.Add）
...
- `Shutdown(ctx)` 流程：
  ...
  4. `gcli.Stop()` 停止 gnet 引擎，等待 event-loop 退出，禁止后续 `asyncWG.Add`
  5. 启动 goroutine 等待 `asyncWG.Wait()`，select `ctx.Done()` 超时
  6. `replyPool.Close()` 清理共享 ReplyPool
...
`gnet.NewClient` 或 `gcli.Start` 失败时，构造路径必须立即关闭已创建的 ReplyPool。

## Dialer 生命周期

`Dialer` 使用同一个生命周期锁串行化 gnet client 的首次创建和 `Close`。`Dial`/`Request` 必须取得局部 client 引用后再拨号，禁止在 `closed` 检查与 `sync.Once` 初始化之间留下 `gcli == nil` 窗口。远端断开时 `OnClose` 必须将返回给调用方的 Session 标记为 closed。
