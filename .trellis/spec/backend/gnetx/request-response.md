...
- `Request()`: `compositeTID := s.sessionID + "|" + msg.TID()` → `pool.Register`
- `resolveResponse()`: `compositeTID := s.sessionID + "|" + tid` → `pool.Resolve`
...
1. `srv.Manager().GetByClientID(clientID)` 获取 Session
