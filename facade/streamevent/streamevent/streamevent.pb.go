...
// SocketIO 上行回调请求。
// socketgtw 在 socket 事件发生时调用该 RPC, 通知下游业务服务处理。
// 业务服务根据 event 类型决定处理逻辑, 如初始化房间、校验权限、处理业务请求等。
//
// event 类型与 payload 结构:
//
//  1. __connection__ — 连接建立后触发
//     payload: {"metadata": {...}}
//     metadata 来自 token claims, 具体 key 由 socketgtw 网关配置决定。
//     返回: 房间名数组 JSON, 如 ["room1","room2"], socketgtw 会自动 JoinRoom。
//
//  2. __disconnect__ — 断开连接时触发
//     payload: {"metadata": {...}}
//     返回: 通常为空。
//
//  3. __join_room_up__ — 浏览器请求加入房间前触发
//     payload: {"metadata": {...}, "room": "房间名"}
//     返回: 通过时正常返回; 拒绝时返回 gRPC 错误, socketgtw 拒绝加入。
//
//  4. __up__ — 浏览器业务上行事件
//     payload: 浏览器原始请求 JSON, 包含 reqId、payload、room(可选)、event(可选)。
//     返回: 业务数据, socketgtw 包装成 SocketResp 返回给浏览器。
//
//  5. 自定义上行事件
//     payload 结构由 socketgtw 对应 handler 决定, 业务服务自行解析。
type UpSocketMessageReq struct {
	...
	// JSON 字符串负载。结构由 event 决定, 详见 UpSocketMessageReq 注释。
...
type UpSocketMessageRes struct {
	...
	// - __join_room_up__: 拒绝时返回 gRPC 错误。
	// - __up__: 返回业务数据, socketgtw 包装成 SocketResp。
	// - __disconnect__: 通常为空。
...
func (x *HandlerPlanTaskEventReq) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
...
func (x *HandlerPlanTaskEventReq) GetPlanPk() string {
	if x != nil {
		return x.PlanPk
	}
	return ""
...
func (x *HandlerPlanTaskEventReq) GetBatchPk() string {
	if x != nil {
		return x.BatchPk
	}
	return ""
